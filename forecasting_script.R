...
RPS_err = matrix(NA, ncol = k, nrow = 9)
log_err = matrix(NA, ncol = k, nrow = 9)
Brier_e = matrix(NA, ncol = k, nrow = 9)
...
for(i in 1:(length(ints)-1)){
  ...
  log_err[3, i] = mean(logscore_vectors(m, preds_RPS, obs))
  log_err[4, i] = mean(logscore_vectors(m, preds_RPS_cov, obs))
  log_err[5, i] = mean(logscore_vectors(m, preds_uni, obs))
  #log_err[6, i] = mean(logscore_vectors(m, preds_per, obs)) #not defined
  log_err[7, i] = mean(logscore_vectors(m, preds_olr, obs))
  log_err[8, i] = mean(logscore_vectors(m, preds_olr_cov, obs))
  log_err[9, i] = mean(logscore_vectors(m, preds_RPS_MLE_cov, obs))
  
  Brier_e[1, i] = mean(BrierScore_vectors(preds_MLE, obs))
  Brier_e[2, i] = mean(BrierScore_vectors(preds_MLE_cov, obs))
  Brier_e[3, i] = mean(BrierScore_vectors(preds_RPS, obs))
  Brier_e[4, i] = mean(BrierScore_vectors(preds_RPS_cov, obs))
  Brier_e[5, i] = mean(BrierScore_vectors(preds_uni, obs))
  Brier_e[6, i] = mean(BrierScore_vectors(preds_per, obs))
  Brier_e[7, i] = mean(BrierScore_vectors(preds_olr, obs))
  Brier_e[8, i] = mean(BrierScore_vectors(preds_olr_cov, obs))
  Brier_e[9, i] = mean(BrierScore_vectors(preds_RPS_MLE_cov, obs))
  
  RPS_err[1, i] = mean(rps_vectors(m, preds_MLE, obs))
  RPS_err[2, i] = mean(rps_vectors(m, preds_MLE_cov, obs))
  RPS_err[3, i] = mean(rps_vectors(m, preds_RPS, obs))
  RPS_err[4, i] = mean(rps_vectors(m, preds_RPS_cov, obs))
  RPS_err[5, i] = mean(rps_vectors(m, preds_uni, obs))
  RPS_err[6, i] = mean(rps_vectors(m, preds_per, obs))
  RPS_err[7, i] = mean(rps_vectors(m, preds_olr, obs))
  RPS_err[8, i] = mean(rps_vectors(m, preds_olr_cov, obs))
  RPS_err[9, i] = mean(rps_vectors(m, preds_RPS_MLE_cov, obs))
}

names = c("jump_MLE", "jump_MLE_cov", "jump_RPS","jump_RPS_cov","Uniform", "Persistence", "olr", "olr_cov", "jump_RPS_MLE_cov")
