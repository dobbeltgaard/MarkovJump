...
Finally, we outline how predictive distributions can be embedded in a stochastic optimization program to plan inspection and maintenance under budget constraints, bridging predictive modeling with decision-making. Although situated in the railway domain, the framework is relevant for other degradation processes where defects are observed irregularly over an ordinal state space.
\end{abstract} 
...
Continuous-time Markov chains have been widely applied in infrastructure degradation modeling for bridges \citep{Mizutani2023RegimeSwitching}, pipelines \citep{JimenezRoa2024Nonhomogeneous}, and drainage systems \citep{Wu2021ConditionPrediction}, making them increasingly relevant for predictive maintenance. 
...
Beyond railway applications, continuous-time Markov chains have been widely used for predictive modeling in other infrastructure domains. \citet{Mizutani2023RegimeSwitching} propose a regime-switching Markov chain for bridge deterioration, improving fit by allowing transitions between distinct aging regimes. \citet{JimenezRoa2024Nonhomogeneous} assess time-inhomogeneous Markov chains for sewer pipe forecasting and show that non-constant transition rates outperform homogeneous models in predictive accuracy. \citet{Wu2021ConditionPrediction} apply Markov chains to drainage assets and demonstrate that including covariates improves forecasts. 
...
Second, while covariate-augmented continuous-time Markov chains have been proposed in related fields \citep{Wu2021ConditionPrediction}, the forecasting impact of covariates has not been systematically quantified in railway degradation contexts.
...
\begin{tabular}{@{}lll@{}}
\toprule
Generator & Allowed jumps & \#Parameters \\\midrule
$\bm{A}_{\RN{1}}$ & Nearest class & $m - 1$ \\$\bm{A}_{\RN{2}}$ & Any forward jump & $m - 1$ \\$\bm{A}_{\RN{3}}$ & Up to 2 classes ahead & $2m - 3$ \\$\bm{A}_{\RN{4}}$ & Up to 3 classes ahead & $3m - 6$ \\$\bm{A}_{\RN{5}}$ & Any forward jump (free) & $m(m-1)/2$ \\\bottomrule
\end{tabular}
\end{table}
...
In the following, we outline and solve a stochastic program that connects probabilistic forecasts to sequential inspection and maintenance decisions over multiple stages.
...
\cref{eq:general_textbook_budget}--\cref{eq:general_textbook_inspection} ensure budget feasibility and at most one action per active defect and stage.
...
The program is implemented and solved for a smaller amount of defects, showing how probabilistic forecasts can be connected to actionable decision-making for infrastructure managers. The authors are hopeful that this could help bridge the gap between predictive modeling and maintenance planning.  Future research could focus on developing more sophisticated decision policies, for example by solving the proposed multistage program in a rolling-horizon setting. Alternative policy classes, such as those described in \citet{powell2018a}, may also be considered.
