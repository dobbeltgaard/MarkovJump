...
%DIF DEL probabilistic_forecasting_v1/latex/main.tex   Tue Feb 24 15:07:27 2026
%DIF ADD probabilistic_forecasting_v2/latex/main.tex   Mon Apr  6 21:09:57 2026
...
%DIF 27c27
%DIF < \usepackage{arydshln}
%DIF -------
%\usepackage{arydshln} %DIF > 
%DIF -------
...
\usepackage{tikz} %DIF > 
\usetikzlibrary{calc,positioning} %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
\usepackage{lineno}
...
\title{Probabilistic Multicategory Forecasting of \DIFdelbegin \DIFdel{Rail }\DIFdelend Defect Progression}
%DIF 60c69
%DIF < \author{Albert Skovgaard Bisgaard, Steven Harrod, Jan Kloppenborg Møller, Bo Friis Nielsen}
%DIF -------
\author{Albert Skovgaard Bisgaard, Marcel Solé Àvila, Steven Harrod, Jan Kloppenborg Møller, Bo Friis Nielsen} %DIF > 
%DIF -------
\date{}
...
\begin{abstract}
\DIFdelbegin \DIFdel{Railway infrastructure faces risks from rail defects that can propagate into rail breaks, jeopardizing safety and reducing network efficiency. 
This paper }\DIFdelend \DIFaddbegin \DIFadd{Defects can be both costly and safety-critical in infrastructure operations. 
This study }\DIFaddend develops a probabilistic \DIFdelbegin \DIFdel{forecasting framework based on a covariate-dependent Markov jump process with }\DIFdelend \DIFaddbegin \DIFadd{framework to predict defect progression based on severity classifications.
This is done in the context of defect records from a large-scale railway network, modeled with Markov jump processes.
Model extensions such as covariate dependence, }\DIFaddend modified sojourn time estimates\DIFdelbegin \DIFdel{to predict defect severity progression. We test alternative generator parameterizations, each varying in structural flexibility, and benchmark them }\DIFdelend \DIFaddbegin \DIFadd{, and mixture distributions are considered alongside multiple generator parameterizations.
We benchmark }\DIFaddend against reference models using proper scoring rules. Forecast diagnostics further assess calibration\DIFdelbegin \DIFdel{and discrimination. The Markov jump process achieves the highest prediction accuracy, with further improvements through ensemble forecasting. 
We demonstrate that accounting for spatial heterogeneity, such as local rail conditions, yields further improvements. }\DIFdelend \DIFaddbegin \DIFadd{, discrimination, and sharpness. 
}\DIFaddend Finally, we outline how predictive distributions can be embedded in a stochastic optimization program to \DIFdelbegin \DIFdel{allocate }\DIFdelend \DIFaddbegin \DIFadd{plan }\DIFaddend inspection and maintenance \DIFdelbegin \DIFdel{actions }\DIFdelend under budget constraints, bridging \DIFdelbegin \DIFdel{statistical modeling with practical infrastructure }\DIFdelend \DIFaddbegin \DIFadd{predictive modeling with }\DIFaddend decision-making.
\DIFdelbegin \DIFdel{The proposed framework advances probabilistic degradation modeling and provides infrastructure managers with a model framework to support predictive maintenance decision-making}\DIFdelend \DIFaddbegin \DIFadd{Although situated in the railway domain, the framework is relevant for other degradation processes where defects are observed irregularly over an ordinal state space}\DIFaddend .
\end{abstract}
...
\DIFdelbegin \DIFdel{Railway infrastructure is }\DIFdelend \DIFaddbegin \DIFadd{Various infrastructure systems are }\DIFaddend critically affected by defects\DIFdelbegin \DIFdel{in the rail component of the track. Cracks initiated in the rail }\DIFdelend \DIFaddbegin \DIFadd{. Defects }\DIFaddend may grow over time and eventually cause \DIFdelbegin \DIFdel{rail fracture.
Rail }\DIFdelend \DIFaddbegin \DIFadd{fractures that make the infrastructure unsafe for operations.
For railway infrastructure, rail }\DIFaddend breaks pose serious safety \DIFdelbegin \DIFdel{risks }\DIFdelend \DIFaddbegin \DIFadd{concerns }\DIFaddend in terms of increased derailment \DIFdelbegin \DIFdel{probability}\DIFdelend \DIFaddbegin \DIFadd{risks}\DIFaddend , leading infrastructure managers to enforce speed restrictions or track replacements when advanced defects are discovered. 
...
To mitigate these risks, infrastructure managers carry out \DIFdelbegin \DIFdel{regular inspections , documenting significant rail deviations }\DIFdelend \DIFaddbegin \DIFadd{inspections where substantial damage can be regarded }\DIFaddend as point defects. 
...
Because maintenance resources are limited, managers often face the challenge of prioritizing \DIFaddbegin \DIFadd{reinspections and maintenance }\DIFaddend among a large number of identified defects.
Defects that pose imminent safety risks should be treated first, but this assessment is difficult for two reasons. 
First, since \DIFdelbegin \DIFdel{rail }\DIFdelend defects evolve over time, the last recorded severity classification may no longer be accurate at the time of decision making.
...
These challenges motivate the development of models capable of forecasting the evolution of defect severity, explicitly accounting for uncertainty and \DIFdelbegin \DIFdel{track-specific }\DIFdelend \DIFaddbegin \DIFadd{defect-specific }\DIFaddend conditions.

We consider this a forecasting problem over an ordinal (and multicategory) state space representing severity classes. Since inspection data are interval-censored, and degradation rates vary with \DIFdelbegin \DIFdel{track-specific }\DIFdelend \DIFaddbegin \DIFadd{defect-specific }\DIFaddend conditions, the modeling problem involves predicting the probability of severity transitions under uncertainty, based on censored transition data and exogenous information.

Accurate forecasts of defect propagation that account for \DIFdelbegin \DIFdel{local track }\DIFdelend \DIFaddbegin \DIFadd{defect-specific }\DIFaddend conditions and inspection intervals can therefore inform timely maintenance decisions and improve both safety and efficiency \citep{Binder2023}. In this setting, a Markov jump process (\DIFaddbegin \DIFadd{MJP; }\DIFaddend also known as a continuous-time Markov chain\DIFdelbegin \DIFdel{, or CTMC)}\DIFdelend \DIFaddbegin \DIFadd{), }\DIFaddend provides a dynamic probabilistic \DIFdelbegin \DIFdel{framework}\DIFdelend \DIFaddbegin \DIFadd{model}\DIFaddend : it yields full predictive distributions over future severity classes and can incorporate spatial variability through covariates \citep{bisgaard2024A}. Unlike point forecasts or expected-time-to-failure models, a full probabilistic forecast explicitly quantifies uncertainty, allowing infrastructure managers to balance risk and cost under resource constraints.

\DIFdelbegin \DIFdel{CTMCs }\DIFdelend \DIFaddbegin \DIFadd{Continuous-time Markov chains }\DIFaddend have been widely applied in infrastructure degradation modeling for bridges \citep{Mizutani2023RegimeSwitching}, pipelines \citep{JimenezRoa2024Nonhomogeneous}, and drainage systems \citep{Wu2021ConditionPrediction}, making them increasingly relevant for predictive maintenance. \DIFdelbegin \DIFdel{Despite their advantages in forecasting, the predictive accuracy of CTMCs is rarely evaluated in railway applications. 
}\DIFdelend %DIF > Despite their advantages in forecasting, the predictive accuracy of CTMCs is rarely evaluated in railway applications. 
...
Beyond railway applications, \DIFdelbegin \DIFdel{CTMCs }\DIFdelend \DIFaddbegin \DIFadd{continuous-time Markov chains }\DIFaddend have been widely used for predictive modeling in other infrastructure domains. \citet{Mizutani2023RegimeSwitching} propose a regime-switching \DIFdelbegin \DIFdel{CTMC }\DIFdelend \DIFaddbegin \DIFadd{Markov chain }\DIFaddend for bridge deterioration, improving fit by allowing transitions between distinct aging regimes. \citet{JimenezRoa2024Nonhomogeneous} assess time-inhomogeneous \DIFdelbegin \DIFdel{CTMCs }\DIFdelend \DIFaddbegin \DIFadd{Markov chains }\DIFaddend for sewer pipe forecasting and show that non-constant transition rates outperform homogeneous models in predictive accuracy. \citet{Wu2021ConditionPrediction} apply \DIFdelbegin \DIFdel{CTMCs }\DIFdelend \DIFaddbegin \DIFadd{Markov chains }\DIFaddend to drainage assets and demonstrate that including covariates improves forecasts. \DIFdelbegin \DIFdel{These studies underscore the importance of model structure in CTMC-based prediction.
}\DIFdelend %DIF > These studies underscore the importance of model structure in CTMC-based prediction.
...
Second, while covariate-augmented \DIFdelbegin \DIFdel{CTMCs }\DIFdelend \DIFaddbegin \DIFadd{continuous-time Markov chains }\DIFaddend have been proposed in related fields \citep{Wu2021ConditionPrediction}, the forecasting impact of covariates has not been systematically quantified in railway degradation contexts.
...
Reliable forecasting of defect severity is essential for prioritizing maintenance under constrained resources. Since severity classifications guide intervention decisions, models that accurately predict how defects will progress can help infrastructure managers decide which defects to reinspect or repair first. However, without systematic evaluation, it is unclear how different modeling assumptions (such as transition structure, state-dependent time scaling, \DIFdelbegin \DIFdel{or exogenous information}\DIFdelend \DIFaddbegin \DIFadd{regression on exogenous information, and mixture distributions}\DIFaddend ) impact predictive performance. To our knowledge, there has been little systematic effort to evaluate predictive performance in probabilistic railway degradation modeling. 
The main contributions of this paper are:

\begin{itemize}
    \item \DIFdelbegin \DIFdel{Forecast evaluation for degradation modeling: We perform scoring-rule-based evaluation of probabilistic forecasts of defect severity, benchmarking the Markov jump process. Beyond log score and RPS, we assess calibration and sharpness through multicategorical reliability diagrams and checkerboard plots.
    }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Exploration of model }\DIFdelend \DIFaddbegin \DIFadd{Generator }\DIFaddend structure: We extend prior \DIFdelbegin \DIFdel{CTMC-based }\DIFdelend \DIFaddbegin \DIFadd{MJP-based }\DIFaddend degradation models by testing alternative parameterizations of the \DIFdelbegin \DIFdel{transition generator, including skip transitions beyond neighboring states.    
    We evaluate how these structural choices affect predictive accuracy. To our knowledge, no prior degradation studies have conducted such an evaluation.
    }\DIFdelend \DIFaddbegin \DIFadd{generator.    
    }\DIFaddend \item Modified sojourn times: \DIFdelbegin \DIFdel{We introduce a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend heuristic extension of the Markov jump process that modifies the \DIFdelbegin \DIFdel{state-wise }\DIFdelend sojourn time estimates \DIFdelbegin \DIFdel{. This preserves the structure of the continuous-time model while allowing greater }\DIFdelend \DIFaddbegin \DIFadd{is introduced. This allows more }\DIFaddend flexibility in representing \DIFdelbegin \DIFdel{heterogeneous }\DIFdelend deterioration dynamics across severity classes.
      \item \DIFaddbegin \DIFadd{Mixture model: A mixture MJP comprised of latent components with corresponding generators is developed to model complex transition behavior in the data.
    }\item \DIFadd{Exogenous dependencies: We test whether adding local track covariates improves forecasts, comparing a global-coefficient regression to a state-dependent-coefficient regression.
     }\item \DIFaddend Optimum score estimation: \DIFdelbegin \DIFdel{We align model estimation }\DIFdelend \DIFaddbegin \DIFadd{Model estimation is aligned }\DIFaddend with the chosen forecast scoring rule\DIFdelbegin \DIFdel{, following the principle of optimum score estimation}\DIFdelend . This approach improves consistency between model \DIFdelbegin \DIFdel{training and evaluation.
    }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Quantification of covariate effects: We assess how including local rail network characteristics (e.g., curvature, tonnage, rail profile) improves predictive accuracy}\DIFdelend \DIFaddbegin \DIFadd{estimation and how predictions are evaluated}\DIFaddend .
     \item \DIFdelbegin \DIFdel{Implementation and reproducibility: We provide an open-source implementation of the model and scoring framework in \texttt{R}/\texttt{C++}, including three methods for computing the transient distribution (uniformization, Padé approximation, and eigendecomposition), designed for flexibility and computational efficiency in applied settings. }\DIFdelend \DIFaddbegin \DIFadd{Forecast evaluation: We perform scoring-rule-based evaluation of the forecasts, benchmarking the Markov jump process across model specifications. Calibration, discrimination, and sharpness are also assessed. %DIF > To our knowledge, no prior degradation studies have conducted such an evaluation.
%DIF >     \item Implementation and reproducibility: An open-source implementation of the model and scoring framework in \texttt{R}/\texttt{C++} is provided, including three methods for computing the transient distribution (uniformization, Padé approximation, and eigendecomposition).
    }\DIFaddend \item \DIFdelbegin \DIFdel{Proposition for use case: We sketch }\DIFdelend \DIFaddbegin \DIFadd{Stochastic optimization program: We demonstrate }\DIFaddend how the predictive distributions can be embedded in a stochastic optimization program that \DIFdelbegin \DIFdel{allocates }\DIFdelend \DIFaddbegin \DIFadd{plans }\DIFaddend inspection and maintenance \DIFdelbegin \DIFdel{actions }\DIFdelend under budget constraints. This connects forecasting to \DIFdelbegin \DIFdel{actionable }\DIFdelend decision-making for infrastructure managers.
\end{itemize}
...
\DIFdelbegin \DIFdel{\Cref{fig:hists_u} }\DIFdelend \DIFaddbegin \DIFadd{\cref{fig:hists_u} }\DIFaddend shows the empirical distribution of observation intervals, conditioned on observed transitions. As seen, the observation intervals are irregular, and all defects either remain in the current class or progress to a more severe one.

\begin{figure}[htb!]
    \centering
    \includegraphics[width = \textwidth]{hists_u_v2.pdf}
    \caption{Histograms of observation intervals in days for all \DIFaddbeginFL \DIFaddFL{observed }\DIFaddendFL defect transitions, given the trajectory $3 \rightarrow 2B \rightarrow 2A \rightarrow 1 \rightarrow 0$. \nth{1} axes are observation intervals in number of days and \nth{2} axes represent relative frequencies wrt. the number of \DIFdelbeginFL \DIFdelFL{jumps }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{transitions }\DIFaddendFL from one category to another. In total 3658 \DIFdelbeginFL \DIFdelFL{jumps }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{transitions }\DIFaddendFL are displayed. The figure is reproduced from \citet{bisgaard2024A}.}
    \label{fig:hists_u}
\end{figure}
...
These variables are selected based on their known relevance to rail degradation \citep{ghofrani2019a,dick2003a,UIC_2002_defecthandbook,RASMUSSEN20178}, and are \DIFdelbegin \DIFdel{min-max scaled }\DIFdelend \DIFaddbegin \DIFadd{transformed }\DIFaddend according to the \DIFdelbegin \DIFdel{procedure }\DIFdelend \DIFaddbegin \DIFadd{procedures }\DIFaddend outlined in \citet{bisgaard2024A}. \DIFaddbegin \DIFadd{This includes min-max scaling without centering, that is,
}\begin{align*}
    \widehat{z} = \frac{z - z_{\text{min}}}{z_{\text{max}} - z_{\text{min}}},
\end{align*}
\DIFadd{where $z$ is the original variable, $\widehat{z}$ is the min-max transformed variable, $z_{\text{min}}$ is the minimum value of $z$, and $z_{\text{max}}$ is the maximum value of $z$. 
%DIF > Covariate multicollinearity diagnostics were assessed using pairwise Pearson correlations and variance inflation factors. All absolute correlations were below $0.6$ and all variance inflation factors were below $1.5$, indicating no evidence of high multicollinearity. If that had been the case, dimensionality reduction techniques could have been used to orthogonalize the covariates (e.g. principal component analysis).
}\DIFaddend 

\DIFaddbegin \DIFadd{Covariate multicollinearity was assessed using pairwise Pearson correlations and variance inflation factors (VIF). All absolute correlations were below $0.6$ and all VIF values were below $1.5$, well below commonly used cut-offs for problematic multicollinearity (e.g. $\text{VIF} > 5$ \citep[Sec. 6.4.1]{sheather2009a}).
...
\DIFaddend % The recorded defect data originate from the Norwegian infrastructure manager Bane Nor. 
...
To assess predictive accuracy, we compare the forecast distribution to the actual observed class at a future time point using proper scoring rules (\DIFdelbegin \DIFdel{\Cref{sec:error_metrics}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:error_metrics}}\DIFaddend ). Well-calibrated forecasts produce probabilities that match observed frequencies across many instances. Additionally, we assess the model’s discrimination ability: its capacity to distinguish between high- and low-risk outcomes (\DIFdelbegin \DIFdel{\Cref{sec:reliability_analysis}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:reliability_analysis}}\DIFaddend ).

As is standard in predictive modeling, we estimate model parameters using one part of the data and evaluate performance on a separate test set. This procedure is described in more detail in \DIFdelbegin \DIFdel{\Cref{sec:prediction_error}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:prediction_error}}\DIFaddend .
...
This section describes the methodological basis to address the prediction task outlined in \DIFdelbegin \DIFdel{\Cref{sec:prediction_task} }\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:prediction_task} }\DIFaddend given the recorded defect severity trajectories in \DIFdelbegin \DIFdel{\Cref{sec:data}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:data}}\DIFaddend . 
We propose the Markov jump process (MJP) for the prediction and modeling task of defect progression\DIFaddbegin \DIFadd{, including different settings for its generator, modifications of sojourn time estimates, regression settings, and mixture distributions}\DIFaddend .
We describe its mathematical properties with computational remarks, if any, and lay out how it is estimated and applied for prediction. In addition, \DIFdelbegin \DIFdel{we describe }\DIFdelend the evaluation methodology \DIFaddbegin \DIFadd{is described }\DIFaddend with respect to error metrics and reference predictors. 
\DIFdelbegin \DIFdel{We denote the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend complete set of rail defects \DIFaddbegin \DIFadd{is denoted }\DIFaddend by $\mathcal{S}$. The set of possible defect classifications is defined as $\mathcal{E} = \{3, 2B, 2A, 1, 0\}$, where $\mathcal{E}$ is ordered with index set denoted $\mathcal{J}$.
...
\DIFdelbegin \DIFdel{To introduce exogenous dependencies to the Markov jump process, we regress the transition rate matrix on the covariates, $\bm{z}^{(s)}$, by
}%DIFDELCMD < \begin{equation}
%DIFDELCMD < \begin{aligned}
%DIFDELCMD <     \bm{A}^{(s)} = \bm{A} \cdot g\left( \bm{\beta}^\top \bm{z}^{(s)} \right)
%DIFDELCMD <     %\lambda_{i,j} ( \bm{z}^{(s)} ) = \lambda_{i,j}^{(0)} \; g\left( \bm{\beta}^\top \bm{z}^{(s)} \right), \quad i,j \in \mathcal{J}
%DIFDELCMD < \end{aligned} \label{eq:trans_rates_covariates}
%DIFDELCMD < \end{equation}
%DIFDELCMD < %%%
\DIFdel{with $\bm{A}$ denoting some base transition rate matrix. The function $g \, : \, \mathbb{R} \rightarrow \mathbb{R_+}$ takes any covariate dot product, $\bm{\beta}^\top \bm{z}^{(s)}$, as input and maps it to the positive real line, ensuring that the transition rates are nonnegative. Example choices could be
}%DIFDELCMD < \begin{align*}
%DIFDELCMD <    g(x) = \exp(x), \quad g(x) = \log(1 + \exp(x) ), \quad \text{and} \quad g(x) = x^2.
%DIFDELCMD < \end{align*}
%DIFDELCMD < %%%
\DIFdel{$\bm{z}^{(s)}$ contains the exogenous information described in \Cref{sec:data}. With this exogeneous information, we deem the Markov chain spatially inhomogeneous as the rail characteristics vary throughout the network as described in \Cref{sec:data}. This dependency setting is indicated by writing $\bm{A}^{(s)}$ with the explicit $s$-dependence.
}\DIFdelend 

\DIFaddbegin \subsubsection{\DIFadd{Transient distribution}} \label{sec:transient_dist}
\DIFaddend Let $\bm{\pi}^{(s)}(t) = \begin{bmatrix} \pi^{(s)}_1(t), \dots, \pi^{(s)}_m(t) \end{bmatrix}$ denote the time-dependent state distribution of defect $s$, a vector of dimension $1 \times m$ satisfying $\sum_{i = 1}^m \pi^{(s)}_i(t) = 1$. For any $t>t_1^{(s)}$, the $i$'th component is given by
\begin{align}
...
Kolmogorov's forward equation, $\Dot{\bm{\pi}}^{(s)}(t) = \bm{\pi}^{(s)}(t) \bm{A}^{(s)}$, governs the change in state probabilities over time, given some initial probabilities \citep[Section 1.3]{bladt_nielsen}. \DIFaddbegin \DIFadd{Remark that the explicit $s$-dependence on $\bm{A}^{(s)}$ is to express that the transition matrix can vary across defects through regression settings.
}\DIFaddend The transient distribution can be obtained by
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD < \begin{aligned}
%DIFDELCMD <                   \bm{\pi}^{(s)}(t_0 + \Delta t) = 
%DIFDELCMD <                   \bm{\pi}^{(s)}(t_0) \exp{ ( \Delta t \bm{A}^{(s)} ) }. \label{eq:markov_eq_solution}
...
%DIF < As described in \Cref{sec:data}, we model the progression of rail defects. 
%DIF > As described in \cref{sec:data}, we model the progression of rail defects. 
...
%DIFDELCMD < \myparagraph{Nearest class}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Nearest class.}} 
\DIFaddend The first proposed parameterization assumes that a defect transition can only happen to a neighboring defect class. The generator can then be expressed as
\begin{align}
...
\DIFdelbegin %DIFDELCMD < \myparagraph{Relaxed Erlang} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Relaxed Erlang.}} 
\DIFaddend We introduce an alternative parameterization that retains the same number of free parameters as the nearest class generator, but permits longer jumps. %In this setting, longer jumps occur with lower transition rates, reflecting accumulated degradation over multiple intermediate classes. 
...
\DIFdelbegin %DIFDELCMD < \myparagraph{Up to 2 classes ahead}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Up to 2 classes ahead.}}
\DIFaddend The \textit{upper bidiagonal} parameterization allows for transitions to both the immediate next class and the class two steps ahead in severity. Compared to the nearest class generator, this formulation introduces additional flexibility, while still restricting long jumps. The generator is given by
\begin{align}
...
\DIFdelbegin %DIFDELCMD < \myparagraph{Up to 3 classes ahead}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Up to 3 classes ahead.}}
\DIFaddend The fourth parameterization, \textit{upper tridiagonal}, allows jumps of up to three severity levels ahead. The generator matrix takes the form
\begin{align}
...
\DIFdelbegin %DIFDELCMD < \myparagraph{Free upper-triangular}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Free upper-triangular.}}
\DIFaddend The last parameterization assumes that a defect can transition to any class of equal or higher severity, without any functional relationship between transition rates. In this setting, the upper triangle of the generator is \textit{free} of structural restrictions. The generator is expressed as
\begin{align}
    \bm{A}_{\RN{5}} =
    \begin{bmatrix} 
...
%DIFDELCMD <  
%DIFDELCMD < %%%
\DIFdelend \subsubsection{Model with modified sojourn time estimates} \label{sec:warping}
...
To allow for such flexibility, we propose a somewhat heuristic model that preserves some of the virtues of the MJP modelling approach, by modifying \DIFdelbegin \DIFdel{sojourn times }\DIFdelend \DIFaddbegin \DIFadd{the sojourn time }\DIFaddend estimates with state-specific weights, $\xi_i$.
...
Given the collection of tuples $\{ ( y_1^{(s)}, \tau^{(s)} ) \}_{s \in \mathcal{S}}$, the expected sojourn \DIFdelbegin \DIFdel{times }\DIFdelend \DIFaddbegin \DIFadd{time }\DIFaddend in state $i$ during the interval $\left[ 0,\tau^{(s)} \right]$ conditional on the initial state $y_1^{(s)}$ is defined as
\DIFdelbegin %DIFDELCMD < \begin{equation*}
%DIFDELCMD <     \begin{aligned}
%DIFDELCMD <     \mu_i^{(s)} &=\mathbb{E} \left[ \left. \int_0^{\tau^{(s)}} \mathds{1}_{\{X^{(s)}(t) = i\}} \; dt \; \Big| \; X^{(s)}(0) = y_1^{(s)} \right. \right] \\ 
%DIFDELCMD < 				&= \int_0^{\tau^{(s)}}\mathds{P}\left(X^{(s)}(t)=i \; \Big| \; X^{(s)}(0)=y_1^{(s)}\right)dt \\ 
%DIFDELCMD < 				&= \int_0^\tau \pi^{(s)}_i(t) \; dt.
%DIFDELCMD < \end{aligned}  \label{eq:cumulative_sojourn_time}
%DIFDELCMD < \end{equation*}%%%
\DIFdelend \DIFaddbegin \begin{equation*}
    \begin{aligned}
...
\end{equation*}\DIFaddend  
This can be calculated \DIFdelbegin \DIFdel{exact}\DIFdelend \DIFaddbegin \DIFadd{exactly}\DIFaddend . Defining $\bm{\mu}^{(s)}  =  \left(\mu_1^{(s)},\dots,\mu_{m-1}^{(s)}\right)$, we have that
\DIFdelbegin %DIFDELCMD < \begin{equation*}
%DIFDELCMD <     \begin{aligned}
%DIFDELCMD < \bm{\mu}^{(s)} &= \bm{e}_{y_{1}^{(s)}} \int_0^{\tau^{(s)}}\exp\left(\bm{S}t\right) dt = \bm{e}_{y_{1}^{(s)}} (-\bm{S})^{-1}\left[\bm{I} - \exp \left( \bm{S}\tau^{(s)} \right) \right] \quad \text{and} \\ 
%DIFDELCMD < \mu_m^{(s)}  &= \tau^{(s)} - \bm{\mu}^{(s)}\bm{1}^\top, \quad \text{with} \\
%DIFDELCMD < \bm{A} &= \left[\begin{array}{cc} \bm{S} & \bm{s} \\
%DIFDELCMD < \bm{0} & 0\end{array}\right], 
%DIFDELCMD <     \end{aligned} 
%DIFDELCMD < \end{equation*}%%%
\DIFdelend \DIFaddbegin \begin{equation}
    \begin{aligned}
...
\end{equation}\DIFaddend 
and \DIFdelbegin \DIFdel{$\bm{e}_{y_{1}^{(s)}} \in \mathbb{N}^{1 \times (m-1)}$ is }\DIFdelend \DIFaddbegin \DIFadd{$\bm{e}_{y_{1}^{(s)}}$ is a $1 \times (m-1)$ basis vector being }\DIFaddend $1$ at the \DIFdelbegin \DIFdel{state }\DIFdelend $y_{1}^{(s)}$-index and zero elsewhere, while \DIFdelbegin \DIFdel{$\bm{1} \in \mathbb{N}^{1 \times (m-1)}$ is }\DIFdelend \DIFaddbegin \DIFadd{$\bm{1}$ is a $1 \times (m-1)$ vector being }\DIFaddend $1$ everywhere.
%DIF < %
\DIFaddbegin 

%DIF >  \in \mathbb{N}^{1 \times (m-1)}$ is $1$ everywhere. 
%DIF > \in \mathbb{N}^{1 \times (m-1)}$ is 

\DIFaddend The modified time $\widetilde{\tau}^{(s)}$ for \DIFdelbegin \DIFdel{crack }\DIFdelend \DIFaddbegin \DIFadd{defect }\DIFaddend $s \in \mathcal S$ is then obtained as a weighted sum of the expected sojourn time estimates
\begin{align}
...
%DIF < If all $\xi_i = 1$, the model reduces to one without rescaling of time, i.e., $\widetilde\tau^{(s)} = \tau^{(s)}$. 
\DIFaddbegin \DIFadd{To avoid scale redundancy between the transition rates in $\bm A$ and the state weights $\bm\xi$, unconstrained working weights $\widetilde{\bm\xi}\in\mathbb{R}_+^{m-1}$ are used, defining the weights $\bm\xi$ via centering on the log-scale, i.e.,
}$$
 \log \xi_i = \log\widetilde{\xi}_i - \frac{1}{m-1}\sum_{j=1}^{m-1}\log\widetilde{\xi}_j, \quad \text{for } i = 1, \dots, m-1.
$$
\DIFadd{This mapping encodes relative acceleration across the states, which implies that $\prod_{i = 1}^{m-1} \xi_i = 1$; thus keeping the transition rates and state weights identifiable during estimation.
}\DIFaddend The weight for the absorbing state\DIFdelbegin \DIFdel{is set }\DIFdelend \DIFaddbegin \DIFadd{, $\xi_m$, is fixed }\DIFaddend to $1$ to ensure that arrival at the absorbing state concludes the degradation process in the modified time domain as well.
\DIFaddbegin 

%DIF > If all $\xi_i = 1$, the model reduces to one without rescaling of time, i.e., $\widetilde\tau^{(s)} = \tau^{(s)}$. 
\DIFaddend We compare models with and without modified sojourn time estimates in terms of predictive accuracy. \DIFdelbegin \DIFdel{For notational simplicity, $\tau'$ denotes elapsed time in both the standard covariate-dependent MJP and its temporally modified version.
}\DIFdelend %DIF > For notational simplicity, $\tau'$ denotes elapsed time in both the standard covariate-dependent MJP and its temporally modified version.


%DIF > Identifiability and constraints. The warped-time specification introduces a scale redundancy between the baseline transition rates in \bm A and the state weights \bm\xi: multiplying \bm\xi by a constant factor yields a proportional rescaling of \widetilde\tau^{(s)}, which is confounded with rescaling \bm A (equivalently, the \lambda-parameters). To enforce identifiability we therefore (i) constrain \xi_i>0 via the reparameterization \xi_i=\exp(\kappa_i) for i=1,\dots,m-1, and (ii) impose a normalization on the transient-state weights by centering the log-weights,
%DIF > > \bar\kappa=\frac{1}{m-1}\sum_{i=1}^{m-1}\kappa_i,\qquad
%DIF > > \xi_i=\exp(\kappa_i-\bar\kappa)\ \ (i=1,\dots,m-1),
%DIF > >
%DIF > which is equivalent to \prod_{i=1}^{m-1}\xi_i=1 (geometric mean one). The absorbing-state weight is fixed at \xi_m=1.
%DIF > This removes the global scale non-identifiability and ensures that \bm\xi only redistributes effective time across transient states rather than changing the overall time scale.
\DIFaddbegin 

\DIFaddend %In practice, this is calculated by 
%$
...
\DIFaddbegin \subsubsection{\DIFadd{Regression settings}} \label{sec:regression} \label{sec:regression_state_dependent}
\DIFadd{As the progression of defects is dependent on local operational and geometric factors, we regress the transition rate matrix on the covariates, $\bm{z}^{(s)}$. This study proposes two ways of doing that; one where regression coefficients apply equally across the state space (here referred to as \textit{global} regression coefficients), and another where regression coefficients are assumed to be state-dependent.
}

\paragraph{\DIFadd{Global regression coefficients.}}
\Citet{bisgaard2024A} \DIFadd{considered this, per defect, by calculating the combined impact from covariates and then multiplying the generator, such that
}\begin{equation}
\begin{aligned}
...
\end{equation}
\DIFadd{where $\bm \beta_j$ is an $p$-sized vector of coefficients for state $j$. Whereas \cref{eq:trans_rates_covariates} resulted in $p$ extra model coefficients, \cref{eq:trans_rates_covariates2} results in $p(m-1)$ extra model coefficients. Note that the reason for not including weights for state $m$ in \cref{eq:trans_rates_covariates2} is that there are no transition dynamics involved, once a defect has progressed into the absorbing state.
}


%DIF > The function $g \, : \, \mathbb{R} \rightarrow \mathbb{R_+}$ takes any covariate dot product, $\bm{\beta}^\top \bm{z}^{(s)}$, as input and maps it to the positive real line, ensuring that the transition rates are nonnegative. Example choices could be
%DIF > \begin{align*}
%DIF >    g(x) = \exp(x), \quad g(x) = \log(1 + \exp(x) ), \quad \text{and} \quad g(x) = x^2.
%DIF > \end{align*}



\subsubsection{\DIFadd{Mixture distributions}} \label{sec:mixture}
\DIFadd{Mixture distributions can be effective in representing complex dependencies in the data \citep{frydman2024,Bisgaard2026}.
...
\DIFadd{Equivalently, the transient distribution at time $t$ is the mixture
}\begin{align}
\bm\pi_{\text{mix.}}^{(s)}(t) = \sum_{g = 1}^G \kappa_g \bm\pi_g^{(s)}(t), \label{eq:mixture}
\end{align}
\DIFadd{where each $\bm\pi_g^{(s)}$ is evolving according to Kolmogorov's forward equation for $s \in \mathcal S$, $\Dot{\bm{\pi}}_g^{(s)}(t) = \bm{\pi}_g^{(s)}(t) \bm{A}^{(s)}_g$ with initial condition $\bm \pi_g^{(s)}(0) = \bm e_{y_1^{(s)}}$ being a $1\times m$ basis vector.
...
%DIF > This study will compare the predictive performance of mixtures of Markov jump processes for the progression of rail defects.


\DIFaddend \subsubsection{Optimum score estimation}\label{sec:optimum_score_estimation}

Let $\bm{\theta}$ \DIFdelbegin \DIFdel{denote }\DIFdelend \DIFaddbegin \DIFadd{be shorthand for }\DIFaddend the set of model parameters\DIFdelbegin \DIFdel{, consisting of }\DIFdelend \DIFaddbegin \DIFadd{. This set comprises }\DIFaddend the transition rates, covariate coefficients, \DIFdelbegin \DIFdel{and }\DIFdelend the coefficients associated with expected sojourn times, \DIFdelbegin \DIFdel{such that $\bm{\theta} = \{ \lambda_{i,j} : i,j \in \mathcal{J}, i \neq j ; \; \beta_k : k = 1,\dots, p ; \; \xi_l : l = 1, \dots, m-1\} $. }\DIFdelend \DIFaddbegin \DIFadd{and the mixture weights; that is, $\{ \lambda_{i,j}^{(g)} : i,j \in \mathcal{J}, i \neq j, g=1,\dots,G ; \; \beta_k : k = 1,\dots, \widetilde{p} ; \; \xi_l : l = 1, \dots, m-1 ;  \; \kappa_g : g=1,\dots,G-1\} $, with $\widetilde{p}$ denoting the number of coefficients related to the covariates. Note that $\bm{\theta}$ changes under the described model settings, depending on the exact specification.
}\DIFaddend %For the Markov jump process, $\bm{\theta}$ is the collection of parameters constituted by the base transition rates, $\lambda_{i,j}^{(0)}$, and the coefficients on the covariates, $\bm{\beta}$.
\DIFaddbegin 

\DIFaddend \citet{Gneiting2007} argues that the choice of a score function, $\mathcal{D}$, should be consistent with the forecast scoring rule used for evaluation as this improves model performance compared to non-tailored estimation practices \citep{friedman1983a,Copas1983}. 
The \DIFdelbegin \DIFdel{overall }\DIFdelend idea of optimum score estimation is that \DIFdelbegin \DIFdel{we choose }\DIFdelend the measure of the goodness-of-fit \DIFaddbegin \DIFadd{is chosen }\DIFaddend in accordance with how \DIFdelbegin \DIFdel{we seek to evaluate }\DIFdelend the performance of the model \DIFaddbegin \DIFadd{is evaluated}\DIFaddend . 
...
\begin{align}
    ...
    \label{eq:mean_score}
\end{align}
Model parameters are then determined \DIFaddbegin \DIFadd{jointly }\DIFaddend by maximizing the score function, i.e.
\DIFdelbegin \DIFdel{$\widehat{\bm{\theta}}_{|\mathcal{S}|} = \argmax_{\bm{\theta}}\mathcal{D}_{|\mathcal{S}|}(\bm{\theta}), $
}\DIFdelend \DIFaddbegin $$\widehat{\bm{\theta}}_{|\mathcal{S}|} = \argmax_{\bm{\theta}}\mathcal{D}_{|\mathcal{S}|}(\bm{\theta}), $$
\DIFaddend where $\widehat{\bm{\theta}}_{|\mathcal{S}|}  \rightarrow \bm{\theta}_{\infty}$ as $|\mathcal{S}| \rightarrow \infty$ with $\bm{\theta}_{\infty}$ denoting the true set of model parameters, assuming regularity conditions and a strictly proper scoring rule.
...
\DIFdelbegin %DIFDELCMD < \begin{equation}
%DIFDELCMD < \begin{aligned}
%DIFDELCMD <                   \ell\left( \bm{\theta}; Y \right) = 
%DIFDELCMD <                   \sum_{s \in \mathcal{S}} \log \left( \mathds{P}\left(X^{(s)}(t_2^{(s)}) = y_2^{(s)} \mid X^{(s)}(t_1^{(s)}) = y_1^{(s)} \right) \right), \quad \text{with} \\ 
%DIFDELCMD <                  \mathds{P}\left(X^{(s)}(t_2^{(s)}) = y_2^{(s)} \mid X^{(s)}(t_1^{(s)}) = y_1^{(s)} \right) = \left[ \bm{\pi}^{(s)}\left( t_2^{(s)} \right)  \right]_{ y_2^{(s)}} =  \left[ \bm{e}_{y_{1}^{(s)}}  \exp \left( \bm{A}^{(s)} \tau'^{(s)} \right) \right]_{ y_2^{(s)}},
%DIFDELCMD <                  % &= \sum_{s \in \mathcal{S}}
%DIFDELCMD <                   %\log\left(   \left[ \bm{\pi}^{(s)}\left( t_2^{(s)} \right)  \right]_{ y_2^{(s)}} \right)
%DIFDELCMD <                   %\\
%DIFDELCMD <                   %&= \sum_{s \in \mathcal{S}}
%DIFDELCMD <                   %\log\left(   \left[ \bm{e}_{y_{1}^{(s)}}  \exp \left( \bm{A}^{(s)} \tau^{(s)} \right) \right]_{ y_2^{(s)}} \right), 
%DIFDELCMD <                   \label{eq:likelihood_mjp}
%DIFDELCMD < \end{aligned}     
%DIFDELCMD < \end{equation}%%%
\DIFdelend \DIFaddbegin \begin{equation}
\begin{aligned}
...
\end{aligned}     
\end{equation}\DIFaddend 
where $\bm{e}_{y_{1}^{(s)}} \in \mathbb{N}^{1 \times m}$ is $1$ at the state $y_{1}^{(s)}$-index and zero elsewhere, that is, it corresponds to the initial distribution $\bm{\pi}^{(s)}(t_1^{(s)})$. The $y_2^{(s)}$-index on the square brackets indicates that we choose the corresponding element of the transient distribution, \DIFdelbegin \DIFdel{$\bm{e}_{y_{1}^{(s)}}  \exp \left( \bm{A}^{(s)} \tau'^{(s)} \right)$}\DIFdelend \DIFaddbegin \DIFadd{$\bm{e}_{y_{1}^{(s)}}  \exp \left( \bm{A}^{(s)} \tau^{(s)} \right)$}\DIFaddend . 
...
The discrete likelihood is then the product of the probabilities of observing the state, $y_2^{(s)}$, after \DIFdelbegin \DIFdel{$\tau'^{(s)}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau^{(s)}$ }\DIFaddend time (modified or not) has passed upon observing $y_1^{(s)}$ for all $s \in \mathcal{S}$.
\DIFdelbegin \DIFdel{\Cref{eq:likelihood_mjp} }\DIFdelend \DIFaddbegin \DIFadd{\cref{eq:likelihood_mjp} }\DIFaddend is an example on a choice of the score function in \cref{eq:mean_score}.
Maximum likelihood estimation corresponds to optimum score estimation based on the \DIFdelbegin \DIFdel{log-score}\DIFdelend \DIFaddbegin \DIFadd{log score}\DIFaddend . It is therefore optimal with respect to forecast evaluation under this rule. Provided another scoring rule, MLE might not be suitable for optimal forecast performance, which is empirically demonstrated by \citet{Gneiting2005} and \citet{gebetsberger2018a}.
This study applies two probabilistic scoring rules, described in \DIFdelbegin \DIFdel{\Cref{sec:error_metrics}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:error_metrics}}\DIFaddend . To improve prediction performance wrt. to both metrics, the optimum score estimation is based on maximizing the sum of the score functions wrt. their respective scoring rules, i.e.
\begin{align}
...
%DIF > To solve the optimization problem in \cref{eq:mean_score_combination}, we apply a Newton–Raphson algorithm with line search using \texttt{nlminb} from \citet{R_stats}. Gradients are obtained as detailed in \cref{sec:computation_remarks}. 

\DIFaddend % Model parameters can be determined by maximum likelihood estimation, i.e. maximining the logarithm of the quantity in \cref{eq:likelihood_mjp}, i.e.
% \begin{align*}
...
\end{itemize} 
In the following, we present two different metrics that will be used for both estimation (as described in \DIFdelbegin \DIFdel{\Cref{sec:optimum_score_estimation}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:optimum_score_estimation}}\DIFaddend ) and for evaluating model predictions.
...
\subsubsection{\DIFdelbegin \DIFdel{Log Score}\DIFdelend \DIFaddbegin \DIFadd{Logarithmic score}\DIFaddend }
The average \DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{logarithmic score (log score) }\DIFaddend is a widely used scoring rule; optimum score estimation based on the \DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{log score }\DIFaddend corresponds to maximum likelihood estimation. The \DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{log score }\DIFaddend evaluates the logarithm of the probability assigned to the observed outcome. In the case of the current study, the mean \DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{log score }\DIFaddend can be expressed as
\begin{align}
...
\end{align}
\DIFdelbegin \DIFdel{\Cref{eq:log_Score} }\DIFdelend \DIFaddbegin \DIFadd{\cref{eq:log_Score} }\DIFaddend is thus equivalent to \cref{eq:likelihood_mjp}.
...
%DIF < Like the log-score, the Brier score treats classes not corresponding to the observed outcome equally, that is by summing over the quadratic errors, making it also insensitive to distance. A perfect forecast is associated with a zero contribution to the Brier score for each defect, $s$. Therefore, it is a loss function.
%DIF > Like the log score, the Brier score treats classes not corresponding to the observed outcome equally, that is by summing over the quadratic errors, making it also insensitive to distance. A perfect forecast is associated with a zero contribution to the Brier score for each defect, $s$. Therefore, it is a loss function.
...
%DIFDELCMD <     \overline{\text{RPS}} =\frac{1}{\vert \mathcal{S} \vert} \sum_{s \in \mathcal{S}} \sum_{k = 1}^m\left( \sum_{i = 1}^k\bm{\pi}^{(s)}_i\left( t_2^{(s)} \right)  - \sum_{i = 1}^k\mathds{1}_{ \{ i \; = \; y_2^{(s)} \} } \right)^2 \label{eq:RPS}
%DIFDELCMD < \end{align}%%%
...
Compared to the \DIFdelbegin \DIFdel{log-score}\DIFdelend \DIFaddbegin \DIFadd{log score}\DIFaddend , RPS punishes probability masses in false outcomes milder if the wrong predictions are located \textit{closer} to the observed outcome. Thus, RPS introduces an ordering to inaccurate predictions in the sense that some \textit{wrong} predictions can be less wrong.
For the purpose of degradation dynamics where there is a structural ordering wrt. defect severity, it can be argued that RPS is more suitable than the \DIFdelbegin \DIFdel{log-score}\DIFdelend \DIFaddbegin \DIFadd{log score}\DIFaddend , if forecasts are interpreted as beliefs about track 
...
Unlike the \DIFdelbegin \DIFdel{log-score}\DIFdelend \DIFaddbegin \DIFadd{log score}\DIFaddend , the RPS reflects that misclassifying a crack by one class is less serious than misclassifying it by several classes. We therefore suggest placing greater emphasis on RPS evaluation in studies of degradation. 
...
For estimation, loss functions such as RPS are minimized (entering with a negative sign in \cref{eq:mean_score_combination}), whereas the \DIFdelbegin \DIFdel{log-score}\DIFdelend \DIFaddbegin \DIFadd{log score}\DIFaddend , as a utility, is maximized.
%DIF < When used for estimation, loss functions like RPS are minimized (hence enter with a negative sign in the score function $\mathcal{D}$), while the log-score, being a utility function, is maximized.
%DIF > When used for estimation, loss functions like RPS are minimized (hence enter with a negative sign in the score function $\mathcal{D}$), while the log score, being a utility function, is maximized.
...
\DIFaddbegin \subsubsection{\DIFadd{Gradients}} \DIFaddend Previous work by \citet{bisgaard2024A} leveraged \texttt{C++} speed in \texttt{R} using \texttt{Rcpp}, and derived algebraic expressions for the gradients used for model estimation, as it was shown to improve its robustness. 
...
Our implementation is flexible: users may choose the scoring rule (\DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{log score }\DIFaddend and/or ranked probability score), select among the five generator parameterizations in \cref{sec:generator_params} \DIFdelbegin \DIFdel{, }\DIFdelend and optionally incorporate \DIFdelbegin \DIFdel{covariates and the }\DIFdelend modified sojourn time estimates (\cref{sec:warping})\DIFaddbegin \DIFadd{, regression settings (\cref{sec:regression}), as well as mixture distribution (\cref{sec:mixture})}\DIFaddend .

\DIFaddbegin \subsubsection{\DIFadd{Numerical optimization}} 
\DIFadd{Optimum score estimation (\cref{sec:optimum_score_estimation}) involves solving the optimization problem $\widehat{\bm{\theta}}_{|\mathcal{S}|} = \argmax_{\bm{\theta}}\mathcal{D}_{|\mathcal{S}|}(\bm{\theta})$ using \cref{eq:mean_score_combination} with $\bm{\theta}$ being shorthand for the set of model parameters, i.e. the transition rates, covariate coefficients, the coefficients associated with expected sojourn times, and/or the mixture weights.
...
Tolerance thresholds defaults to the ones set by \texttt{nlminb}.}\footnote{\DIFadd{These include a relative improvement threshold for the objective function ($10^{-10}$), a parameter change threshold ($1.5 \times 10^{-8}$), and an additional false convergence tolerance ($2.2\times 10^{-14}$).}}
\DIFadd{Convergence is assessed using the convergence code supplied by the \texttt{nlminb} optimizer, with supplemental diagnostics calculating the gradient L2-norm in the obtained solution.
}




\subsubsection{\DIFadd{Calculation of the matrix exponential}} \DIFaddend Prediction is performed by applying the solution to the Kolmogorov differential equation in \cref{eq:markov_eq_solution} using the aforementioned scoring rules for assessment of accuracy.
...
\DIFaddbegin \subsubsection{\DIFadd{Speed of likelihood computation}} \DIFaddend We have tested the speed of the likelihood computation with respect to the different score options and the different methods for calculating the transient distribution of the Markov chain, using the \texttt{bench} library \citep{R_bench}. We compare our \texttt{C++} implementation with a standard (``slow'') \texttt{R} implementation based on a Padé approximation of the transient distribution. \DIFdelbegin \DIFdel{\Cref{tab:mjp_score_computation_speed} }\DIFdelend \DIFaddbegin \DIFadd{\cref{tab:mjp_score_computation_speed} }\DIFaddend displays the results of the speed investigation.
\begin{table}[htb!]
    \centering
...
\end{tabular}
\end{table}
The results in \DIFdelbegin \DIFdel{\Cref{tab:mjp_score_computation_speed} }\DIFdelend \DIFaddbegin \DIFadd{\cref{tab:mjp_score_computation_speed} }\DIFaddend show a substantial speedup when moving from a standard \texttt{R} implementation to \texttt{C++}.
...
For the \texttt{C++} implementations, computation speed differs little between the \DIFdelbegin \DIFdel{log-score }\DIFdelend \DIFaddbegin \DIFadd{log score }\DIFaddend and RPS, as both require similar operations. By contrast, the standard \texttt{R} implementations differ substantially. This is likely due to the \texttt{cumsum} function used in the RPS implementation, which benefits from sequential memory access and CPU cache optimization.
...
Reference predictors provide a benchmark against which the MJP from \DIFdelbegin \DIFdel{\Cref{sec:mjp} }\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:mjp} }\DIFaddend can be evaluated. They allow us to quantify the benefits of introducing temporal dynamics, state dependence, and covariates relative to simpler predictive models.
In this study, we consider \DIFdelbegin \DIFdel{two such benchmarks. }\DIFdelend \DIFaddbegin \DIFadd{three such benchmarks: 1) }\DIFaddend The cumulative link model (CLM) serves as the primary \DIFdelbegin \DIFdel{, competitive reference predictor , while the }\DIFdelend \DIFaddbegin \DIFadd{reference predictor suited for static regression problems with ordinal state space; 
2) a discrete-time Markov chain (DTMC) serves as a predictor of empirical transition patterns; 
3) a }\DIFaddend uniform distribution predictor provides a minimal baseline that distributes probability evenly across feasible states.

\subsubsection{Cumulative link model} \label{sec:cumulative_link_model}

The \DIFdelbegin \DIFdel{cumulative link model (CLM ) }\DIFdelend \DIFaddbegin \DIFadd{CLM }\DIFaddend is a regression model for ordinal outcomes and serves as this study's primary reference predictor. It provides a static benchmark in which temporal dynamics are not modeled explicitly but are instead encoded through covariates. The covariate design matrix $\bm{z}'^{(s)}$ includes \DIFdelbegin \DIFdel{(}\DIFdelend i) the time between observations, $\tau^{(s)}$, \DIFdelbegin \DIFdel{(}\DIFdelend ii) rail characteristics as described in \DIFdelbegin \DIFdel{\Cref{sec:data}, and (}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:data}, and }\DIFaddend iii) indicator variables for the initial defect state $y_1^{(s)} =2,3,\dots,m$. 
...
Note the staticity is indicated by writing $X^{(s)}$, while we in \DIFdelbegin \DIFdel{\Cref{sec:mjp} }\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:mjp} }\DIFaddend denoted the process by $X^{(s)}(t)$ with explicit time dependence.
...
In the prediction study in \DIFdelbegin \DIFdel{\Cref{sec:results}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:results}}\DIFaddend , we test the logit, probit, and complementary log-log link functions to evaluate which yields the highest prediction accuracy.
...
With determined model coefficients, the \DIFdelbegin \DIFdel{cumulative link model }\DIFdelend \DIFaddbegin \DIFadd{CLM }\DIFaddend can be applied for prediction. 
...
%DIF > \subsubsection{Random forest} \label{sec:random_forest}
%DIF > Random forests (RF) are an ensemble of decision trees and can be used for multiclass classification taks \citep{Breiman2001}. 
%DIF > Like the case of the CLM, the second defect recording $ y_2^{(s)}\in\{1,\dots,m\}$ constitutes the categorical response, and $ \bm z'^{(s)}$ is used as covariates (i.e. starting state, observation interval, and optionally the exogenous information described in \cref{sec:data}).
%DIF > Each decision tree is trained on a bootstrap resample and uses random subsampling at each split. The resulting probabilistic forecast is obtained from the empirical vote proportions across trees,
%DIF > \[
%DIF > \widehat{\pi}^{(s),\text{RF}}_j(\bm z'^{(s)}) \;=\; \frac{1}{B}\sum_{b=1}^B \mathds{1}\{T_b(\bm z'^{(s)}) = j\}, \quad j=1,\dots,m,
%DIF > \]
%DIF > where $T_b(\cdot)$ denotes the class prediction of tree $b$ and $B$ is the number of trees.
\DIFaddbegin 

%DIF > Although random forests do not explicitly enforce ordinality, their probabilistic forecasts can be assessed using the same proper scoring rules as the other predictors.


\subsubsection{\DIFadd{Discrete-time Markov chain}} \label{sec:DTMC}
\DIFadd{Like MJPs, DTMCs can be used to model dynamic processes. Contrary to the MJP, the random variable in a DTMC is indexed at discrete time points. 
...
\end{align}
\DIFadd{where $\bm \Gamma$ denotes the transition probability matrix and $\Gamma_{ij}$ its $(i,j)$'th entries.
...
$$
\DIFadd{where $\alpha$ is a non-negative smoothing parameter.
}

\DIFadd{Compared with the CLM, the DTMC provides a benchmark that estimates transition dynamics directly from data.
}



\DIFaddend \subsubsection{Uniform distribution predictor} \label{sec:uniform_predictor}
...
\section{Results and \DIFdelbegin \DIFdel{discussion}\DIFdelend \DIFaddbegin \DIFadd{perspectives}\DIFaddend } \label{sec:results}
We evaluate the predictive accuracy of the \DIFdelbegin \DIFdel{MJP }\DIFdelend \DIFaddbegin \DIFadd{MJPs }\DIFaddend under the model settings from \DIFdelbegin \DIFdel{\Cref{sec:mjp}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:mjp}}\DIFaddend , compare against the reference predictors in \DIFdelbegin \DIFdel{\Cref{sec:reference_forecasts}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:reference_forecasts}}\DIFaddend , and analyze differences in \DIFdelbegin \DIFdel{both prediction error and distributional behavior}\DIFdelend \DIFaddbegin \DIFadd{transient distributions and transition residuals}\DIFaddend . In addition to error scores, \DIFdelbegin \DIFdel{we assess }\DIFdelend calibration and discrimination \DIFaddbegin \DIFadd{of the forecasts are assessed }\DIFaddend using reliability diagrams and checkerboard plots\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and sharpness through an entropy measure. Further, we propose a use case for the forecasting models, i.e., a decision making framework taking the probabilistic forecasts as inputs, demonstrating how prediction methods can be incorporated in maintenance and inspection planning.
}\DIFaddend 

\DIFaddbegin \DIFadd{\cref{fig:flowchart} provides a flowchart of this present study, displaying how its different parts  are connected.
}
...
\caption{\DIFaddFL{Workflow of the present study.}} \label{fig:flowchart}
\end{figure}


\DIFaddend \subsection{Prediction \DIFdelbegin \DIFdel{errors}\DIFdelend \DIFaddbegin \DIFadd{accuracy}\DIFaddend } \label{sec:prediction_error}
The \DIFdelbegin \DIFdel{prediction errors are calculated using }\DIFdelend \DIFaddbegin \DIFadd{predictions will be evaluated using the metrics RPS and log score (see \cref{sec:error_metrics}). This section outlines the procedure for how this has been done, including obtained diagnostics from the model estimation. Finally, model performances are compared based on the computed measures of predictive accuracy.
}

\subsubsection{\DIFadd{Out-of-sample evaluation via cross-validation}} \label{sec:CV}
\DIFadd{The calculation of prediction accuracy is done within }\DIFaddend a $k$-fold cross-validation \DIFdelbegin \DIFdel{procedure, }\DIFdelend \DIFaddbegin \DIFadd{(CV) procedure, so that the predictions are evaluated \textit{out-of-sample}. This will be }\DIFaddend described in the following.

\begin{enumerate}
    \item \DIFdelbegin \DIFdel{Choose a $k$. The larger $k$ is chosen, the higher the computational burden becomes \citep{fushiki2011a}. We use $k = 5$ in this study.
    }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{By }\DIFdelend \DIFaddbegin \DIFadd{At }\DIFaddend random, divide \DIFdelbegin \DIFdel{$\mathcal{S}$ }\DIFdelend \DIFaddbegin \DIFadd{the set of defects }\DIFaddend into $k$ subsets of approximately equal size \DIFaddbegin \DIFadd{given a fixed seed. To avoid leakage (i.e. that defects coming from nearby locations are not in both the training and test set), define a group identifier $g(s) = (l^{(s)},\left \lfloor{p^{(s)}/\Delta}\right \rfloor )$, with $l^{(s)}$ denoting the rail line of defect $s$, $p^{(s)}$ its kilometric position, and $\Delta$ the tolerance threshold for spatial overlap. Let $\mathcal{G} = \{g(s) : s \in \mathcal S\}$ denote the set of unique groups of defects. We then construct a random partition $\{\mathcal{G}_1, \dots, \mathcal{G}_k\}$ (approximately equal in number of groups) and define $\mathcal S_i = \{s : g(s) \in \mathcal G_i\}$}\DIFaddend . 
    \DIFdelbegin \DIFdel{To ensure reproducibility, a seed for the random number generator should be chosen.
    }\DIFdelend \item For each of the $k$ subsets (folds):
    \begin{enumerate}
        ...
        \item Model estimations are performed on the set $\mathcal{S}_{\neg i}$. The parameters of the Markov jump process are determined as described in \DIFdelbegin \DIFdel{\Cref{sec:optimum_score_estimation}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:optimum_score_estimation}}\DIFaddend , and the parameters of the cumulative link model are determined as described in \DIFdelbegin \DIFdel{\Cref{sec:cumulative_link_model}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:cumulative_link_model}}\DIFaddend .
        \item The models are used to produce predictions of the distributions of \DIFdelbegin \DIFdel{$\bm{\pi}^{(s)}\left( t_2^{(s)}\right)$ }\DIFdelend \DIFaddbegin \DIFadd{$\widehat{\bm{\pi}}^{(s)}\left( t_2^{(s)}\right)$ }\DIFaddend for the defects $s \in \mathcal{S}_i$ provided their corresponding initial state $y_1^{(s)}$, their initial time stamp $t_1^{(s)}$, and the covariates $\bm{z}^{(s)}$ (for the Markov jump process) or $\bm{z}'^{(s)}$ (for the cumulative link model).
        \item If $i < k$, then increase $i = i + 1$ and GOTO (a); else GOTO \DIFdelbegin \DIFdel{4.
    }\DIFdelend \DIFaddbegin \DIFadd{3.
    }\DIFaddend \end{enumerate}
    \item For each model with the corresponding set of predictions, compute the \DIFdelbegin \DIFdel{average prediction errors }\DIFdelend \DIFaddbegin \DIFadd{prediction scores }\DIFaddend given all end point observations $y_2^{(s)}$ for $s \in \mathcal{S}$ for each metric described in \DIFdelbegin \DIFdel{\Cref{sec:error_metrics}}\DIFdelend \DIFaddbegin \DIFadd{\cref{sec:error_metrics}}\DIFaddend .
\end{enumerate}

\DIFdelbegin \DIFdel{We assess the predictive performance of each generator parameterization and assess how exogenous rail characteristics and the modification to sojourn time estimates affect prediction accuracy.
We use the link function, }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Computational settings and recorded diagnostics}} \label{sec:CV_details}
\DIFadd{The larger $k$ is chosen in the CV, the higher the computational burden becomes \citep{fushiki2011a}. In this study, $k=5$ is used, which given the procedure in \cref{sec:CV} results in folds containing \num{703}, \num{725}, \num{751}, \num{752}, and \num{727} defects, respectively.}\footnote{\DIFadd{The seed for the CV is set at \texttt{1}.}} \DIFadd{The threshold for the spatial overlap is $\Delta = 0.1$ in kilometers.
}

\DIFadd{For the DTMC (cf. \cref{sec:DTMC}), we use $\alpha=0.1$ and discretize the observation interval $\tau$ (in years) using the bin edges
$
0,0.05,0.10,\dots,3.50,\infty
$.
For the regressions in \cref{eq:trans_rates_covariates,eq:trans_rates_covariates2}, the link function }\DIFaddend $g(x) = \exp(x)$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{is used}\footnote{\DIFadd{Using $g(x)=\log(1+\exp x)$ (softplus) gave similar results as the exponential link. The square link $g(x)=x^2$ was less numerically robust in our experiments, likely because it is not an injective mapping.}} \DIFadd{across all MJP specifications.
...
\DIFadd{Starting values for the parameters in the numerical optimization are as follows: 1) all free transition rates parameters $\lambda_{i,j}^{(g)}$ }\DIFaddend for \DIFdelbegin \DIFdel{\cref{eq:trans_rates_covariates}. As outlined in \Cref{sec:cumulative_link_model}, we benchmark against cumulative link models (CLMs)}\DIFdelend \DIFaddbegin \DIFadd{$g = 1,\dots,G$ were initialized at $\exp(-1)$ given any generator parameterization; 2) all regression coefficients $ \beta_k$ for $ k = 1,\dots, \widetilde{p}$ were initialized at $0$ where $\widetilde{p}$ denotes the number of covariate coefficients; 3) the sojourn time weights were initialized to $\xi_i=1$ }\DIFaddend with \DIFdelbegin \DIFdel{logistic, probit, and complementary log-log link functions. As with the MJP}\DIFdelend \DIFaddbegin \DIFadd{$\xi_m$ fixed at $1$ (using the working parameters $\widetilde \xi$ in \cref{sec:warping}); and 4) for mixture MJPs, the unconstrained mixture weights were initialized equally (implying $\tilde\kappa_g = 1$ for $g=1,\dots,G-1$ with $\tilde\kappa_G=1$), which corresponds to uniform mixture weights $\kappa_g = 1/G$ under the normalization $\kappa_g=\tilde\kappa_g/\sum_{i=1}^G \tilde\kappa_i $ (\cref{sec:mixture}).  
...
\DIFadd{Across folds, estimation diagnostics are recorded; Appendix \ref{sec:diagnostics_estimation} provides a fold-wise overview of the gradient norms of the objective function in \cref{eq:mean_score_combination} and Akaike information criterion (AIC)}\footnote{\DIFadd{AIC is calculated as $\text{AIC} = 2 (\text{dim}_{\bm \theta} - \ell (\widehat{\bm \theta}  ; \mathcal{S}_i) )$ for fold $i$. Note that $\widehat{\bm \theta}$ are determined using the objective function in \cref{eq:mean_score_combination}, but AIC is computed from the log-likelihood, $\ell$, evaluated at $\widehat{\bm\theta}$.}} \DIFadd{given the obtained model parameters for each fold and MJP specification.
Appendix \ref{sec:diagnostics_estimation} shows that the numerical optimization is generally stable across folds and models, with small gradient norms at the obtained solutions. The optimizer’s convergence codes indicate convergence for all models on all folds, except three cases of the mixture MJP, that is: \(\bm{A}_{\RN{1}}\) with covariates on fold 2}\DIFaddend , \DIFdelbegin \DIFdel{we also investigate the effect of including exogenous rail characteristics. The results from the described procedure are presented in \Cref{tab:forecast_error_tab} (page \pageref{tab:forecast_error_tab})}\DIFdelend \DIFaddbegin \DIFadd{\(\bm{A}_{\RN{2}}\) with covariates on fold 5, and \(\bm{A}_{\RN{4}}\) without covariates on fold 1 (also reflected by the gradient norms). For these cases, convergence could be reached by adjusting the initialization; but for consistency, we report results based on the initialization described above. As will appear in \cref{sec:prediction_scores_tables}, the AIC values in Appendix \ref{sec:diagnostics_estimation} are by and large lowest for the models with the highest predictive accuracy.
%DIF > Subsequently (and because this study focuses on predictive performance), emphasis will be placed on the predictive scores rather than AIC in the model evaluation.
%DIF > %#187       gerlang_cov1_exp_exp_all_no_warp_mixture4    2         
%DIF > %#260 gerlang_relax_cov1_exp_exp_all_no_warp_mixture4    5        
%DIF > %#296   tridiagonal_cov0_exp_exp_all_no_warp_mixture4    1    
}

\subsubsection{\DIFadd{Summary of predictive scores across models}} \label{sec:prediction_scores_tables}
\DIFadd{\cref{tab:prediction_scores} reports the mean predictive scores across folds from the cross-validation procedure (\cref{sec:CV,sec:CV_details}), together with their standard deviations across folds}\DIFaddend .
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Prediction errors from $k$-fold cross-prediction procedures in terms of average ranked probability score ($\overline{\text{RPS}}$) and average log score ($\overline{\text{LogS}}$). ``Exogeneous information'' refers to whether or not the rail characteristics described in \Cref{sec:data} are included in the model.}}
%DIFAUXCMD
%DIFDELCMD < \input{forecast_error_tab}
%DIFDELCMD <     \label{tab:forecast_error_tab}
%DIFDELCMD <     %%%
%DIF < remember to manually add: \cdashline{2-7} \addlinespace[0.3em]
    %DIF < between different parameterizations of OLR and MJP
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \input{forecast_error_tab_tot}
\DIFaddend 


\DIFaddbegin \paragraph{\DIFadd{Reference predictors.}}
\DIFaddend Among the CLMs, the logistic link achieves the best scores, while the complementary log-log link performs worst. Although the logistic assumption underlies the latent variable, this should not be interpreted as evidence that defect propagation itself follows a logistic distribution\DIFdelbegin \DIFdel{—}\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend only that it provides the most effective link function in this context. 
\DIFaddbegin \DIFadd{In comparison, the DTMC performs similar as the logistic link CLM in terms of RPS, but substantially better in terms of the log score.
...
\DIFdelbegin \DIFdel{CLMs provide strong static baselines, but }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Score differences.}}
\DIFadd{The }\DIFaddend MJPs consistently outperform \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{the CLMs }\DIFaddend in log score, reflecting the advantage in capturing temporal dynamics and state dependence. For RPS, the relative difference is smaller: while MJPs are favored in most cases (and especially for the more complex \DIFdelbegin \DIFdel{generators), parameterizations $\bm{A}{\RN{1}}$ and $\bm{A}{\RN{2}}$ without modified sojourn time estimates slightly }\DIFdelend \DIFaddbegin \DIFadd{model specifications), the parameterizations $\bm{A}_{\RN{1}}$ and $\bm{A}_{\RN{2}}$ with either no exogenous information or global regression }\DIFaddend underperform compared to the logistic CLM. This \DIFdelbegin \DIFdel{pattern is expected: the }\DIFdelend \DIFaddbegin \DIFadd{could suggest that these MJPs are sharper but less well calibrated than the CLM, because the }\DIFaddend log score is \DIFdelbegin \DIFdel{particularly sensitive to forecast sharpness and the ability to assign high probability to the realized outcome}\DIFdelend \DIFaddbegin \DIFadd{more sensitive to concentration of probability on the realized category}\DIFaddend , while the RPS \DIFdelbegin \DIFdel{also rewards calibration }\DIFdelend \DIFaddbegin \DIFadd{rewards good probabilistic performance }\DIFaddend across the ordinal scale \DIFdelbegin \DIFdel{. 
This reflects the trade-off between sharpness and calibration: the nearest class MJP exhibits stronger discrimination but weaker calibration compared to the logistic CLM.
%DIF < Consequently, the MJP with the generalized Erlang parameterization demonstrates stronger discrimination ability but weaker calibration relative to the CLM with logistic. %A similar trade-off is observed between the ensemble forecast and the free upper parameterization $\bm{A}'''^{(s)}$.
}\DIFdelend \DIFaddbegin \DIFadd{more so than the log score.
}\DIFaddend However, in almost all cases the two \DIFdelbegin \DIFdel{different error }\DIFdelend \DIFaddbegin \DIFadd{probabilistic }\DIFaddend scores show the same tendencies in the prediction accuracy \DIFdelbegin \DIFdel{of the various models, making it more }\DIFdelend \DIFaddbegin \DIFadd{across the models, which makes it }\DIFaddend difficult to comment on \DIFdelbegin \DIFdel{which models are more calibrated or have better }\DIFdelend \DIFaddbegin \DIFadd{deficiencies on sharpness, calibration, and }\DIFaddend discrimination ability. \DIFdelbegin \DIFdel{We therefore provide a }\DIFdelend \DIFaddbegin \DIFadd{Also, relative differences in RPS and log score alone are not enough to evaluate these forecast qualities. A }\DIFaddend detailed diagnostic analysis \DIFdelbegin \DIFdel{in \Cref{sec:reliability_analysis}}\DIFdelend \DIFaddbegin \DIFadd{is therefore provided in \cref{sec:reliability_analysis}}\DIFaddend .

\DIFaddbegin \paragraph{\DIFadd{Generator parameterizations.}}
\DIFaddend The choice of generator parameterization substantially impacts prediction accuracy. 
\DIFdelbegin \DIFdel{Overall, the }\DIFdelend \DIFaddbegin \DIFadd{The structural differences between the generator parameterizations reflect different assumptions about the physical degradation mechanisms of rail defects.
In summary, $\bm{A}_{\RN{1}}$ enforces gradual progression via neighboring classes, }\DIFaddend $\bm{A}_{\RN{4}}$ \DIFaddbegin \DIFadd{(and similarly $\bm{A}_{\RN{3}}$ }\DIFaddend and $\bm{A}_{\RN{5}}$\DIFdelbegin \DIFdel{parameterizations yield the best scores, which indicates that greater model flexibility improves forecast performance for this prediction challenge.
In \Cref{sec:results_transient_dist_markov}}\DIFdelend \DIFaddbegin \DIFadd{) allows for abrupt jumps to severe states (also $\bm{A}_{\RN{2}}$ but more constrained). 
...
For the standard MJP}\footnote{\DIFadd{``Standard MJP'' refers to the MJP without modified sojourn time estimates, mixture distributions, either without covariates or else using the global regression setting (\cref{eq:trans_rates_covariates}).}}\DIFadd{, predictive accuracy increases with generator flexibility, so that $\bm{A}_{\RN{4}}$ and $\bm{A}_{\RN{5}}$ are the best performing parameterizations within that model class. For the more complex model structures (the models with modified sojourn estimates, state-dependent regression coefficients, mixture distributions), the improvements are either smaller or non-existent when moving from $\bm{A}_{\RN{3}}$ to more flexible parameterizations (also supported by the AIC values from Appendix \ref{sec:diagnostics_estimation}).
}

\DIFadd{In practice, selecting a generator parameterization could depend on the characteristics of the physical degradation mechanisms and data characteristics. For instance, if domain knowledge suggests that degradation mostly progresses gradually, neighbor-to-neighbor transitions can be modeled using $\bm{A}_{\RN{1}}$. If sudden damage is expected to happen, then transitions over multiple classes could be allowed in the generator.
...
\DIFadd{In \cref{sec:results_transient_dist_markov}}\DIFaddend , we further compare and discuss the transient distributions from \DIFdelbegin \DIFdel{chosen exampleson parameterizations}\DIFdelend \DIFaddbegin \DIFadd{selected examples}\DIFaddend .



%DIF < also write which numerical optimizer was used
%DIF < Among all models in \Cref{tab:forecast_error_tab}, the free upper Markov jump process achieves the highest log score, while the ensemble forecast achieves the best ranked probability score and Brier score.
\DIFaddbegin \paragraph{\DIFadd{Model with modified sojourn time estimates, mixture distributions, and ensemble forecast.}}
\DIFaddend The model with modified sojourn time estimates (see \cref{sec:warping}) \DIFdelbegin \DIFdel{consistently }\DIFdelend improves prediction accuracy across all parameterizations \DIFdelbegin \DIFdel{, indicating }\DIFdelend \DIFaddbegin \DIFadd{compared to the standard MJP. This indicates }\DIFaddend that modifying sojourn time estimates can correct for biases introduced by severity classification schemes\DIFdelbegin \DIFdel{that do not necessarily align with underlying physical crack sizes}\DIFdelend \DIFaddbegin \DIFadd{, because the effective clock runs faster or slower depending on the occupied class through \cref{eq:warped_time}, which can compensate for defect classifications not being proportional to the physical crack size}\DIFaddend .

\DIFdelbegin \DIFdel{Remark: The $\bm{A}_{\RN{2}}$ parameterization is not included under the model with modified sojourn time estimates, as estimation did not converge. This could be due to }\DIFdelend \DIFaddbegin \DIFadd{The mixture MJPs yield the best predictive scores when comparing each generator parameterization across the other model specifications. Even though it introduces many additional model parameters to be determined, the mixture MJPs have the lowest AIC values (Appendix \ref{sec:diagnostics_estimation}). The largest improvements are seen in terms of }\DIFaddend the \DIFdelbegin \DIFdel{interaction between the harmonic skip of $\bm{A}_{\RN{2}}$ and the weights of the sojourn time modification, creating identifiability issues. }\DIFdelend \DIFaddbegin \DIFadd{log score.
}\DIFaddend 

\DIFdelbegin \DIFdel{Including exogenous track characteristics likewise reduces prediction error across all investigated model settings of both the MJP and CLM }\DIFdelend \DIFaddbegin \DIFadd{The ensemble forecast is among the best-performing predictors, consistent with prior findings that model averaging can improve predictive accuracy \citep{wu2021a}. The structural differences between predictors allow them to capture complementary aspects of the data. By pooling their predictions, ensemble averaging reduces idiosyncratic model errors and can improve calibration. In our experiments, it does not outperform any of the mixture MJPs with covariates.
}


\paragraph{\DIFadd{Impact of introducing exogenous information.}}
\DIFadd{Across CLM and MJP settings, we see that including exogenous information improves forecast accuracy}\DIFaddend . This is consistent with \citet{bisgaard2024A}, who found that such covariates explain structural variability between rail lines, \DIFdelbegin \DIFdel{and supports }\DIFdelend \DIFaddbegin \DIFadd{supporting }\DIFaddend the notion that \DIFdelbegin \DIFdel{the rail network is }\DIFdelend \DIFaddbegin \DIFadd{rail networks are }\DIFaddend heterogeneous and that local conditions at the \DIFdelbegin \DIFdel{crack location }\DIFdelend \DIFaddbegin \DIFadd{defect locations }\DIFaddend must be accounted for\DIFaddbegin \DIFadd{, see for example \citet{bai2017a}.
However, this improvement is relatively modest compared with the improvements obtained when moving from one model class to another. For example, the gain from introducing exogenous information into the CLM is smaller than the gain from moving from either CLM type to the DTMC. This suggests that, although defect transition behavior depends on the included covariates, forecast accuracy is improved more by using a model class that better captures the transition behavior in the data. Although there is one caveat to that conclusion: by introducing state-dependent regression coefficients (via \cref{eq:trans_rates_covariates2}), \cref{tab:prediction_scores} shows that predictive accuracy is improved rather substantially across the different MJP generator parameterizations. One reason for this is that \cref{eq:trans_rates_covariates2} incorporates the expected sojourn time estimates $\mu_j^{(s)}$ through the weights $w_j^{(s)}$, which we saw earlier improved the forecast accuracy in the models with modified sojourn time estimates, but even more so for the state-dependent regression setting. What this regression setting also does is to allow covariates to impact transition rates conditional on how progressed defects are, thereby modeling a more complex dependency between transition behavior and covariates. The cost is more parameters, but in this forecasting problem, it improves out-of-sample accuracy despite that}\DIFaddend .


\DIFdelbegin \DIFdel{The ensemble forecast achieves strong performance, consistent with prior findings thatmodel averaging improves predictive accuracy \citep{wu2021a}. The structural differences between predictors allow them to capture complementary aspects of the data. By pooling their predictions, ensemble averaging reduces idiosyncratic model errors and improves calibration.
 }%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < We next report findings related to the choice of link function in the regression of transition rates within the Markov jump process (see \Cref{eq:trans_rates_covariates}).
%DIF < To avoid an overly large table, we only include results for the softmax link function in \Cref{tab:forecast_error_tab}. However, among the link functions tested, the softmax link achieved the best predictive performance, followed by the exponential link, with the square link performing worst, although the differences were relatively minor.
%DIF < This ordering aligns with expectations: the softmax link is less numerically sensitive than the exponential link, while the square link lacks injectivity, which could introduce ambiguity in the numerical optimization algorithm.
%DIFDELCMD < 
...
%DIFDELCMD <   
%DIFDELCMD <  \input{transient_dists}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{\DIFdelbegin \DIFdel{Transient distributions of the Markov chains}\DIFdelend \DIFaddbegin \DIFadd{Comparing model dynamics}\DIFaddend } \label{sec:results_transient_dist_markov} 
The \DIFdelbegin \DIFdel{generator parameterizations }\DIFdelend \DIFaddbegin \DIFadd{investigated models }\DIFaddend imply substantially different degradation dynamics. In \DIFdelbegin \DIFdel{\Cref{fig:transient_dists}, %DIF < (page \pageref{fig:transient_dists}), 
we illustrate the }\DIFdelend \DIFaddbegin \DIFadd{this section, we compare selected predictors to understand how they evolve over time, approximate the observed transition behavior, and the influence of the covariates.
}

\subsubsection{\DIFadd{Transient distributions}}

\DIFadd{\cref{fig:transient_dists} (page \pageref{fig:transient_dists}) illustrates the }\DIFaddend transient distributions over \DIFdelbegin \DIFdel{an eight-year }\DIFdelend \DIFaddbegin \DIFadd{a ten-year }\DIFaddend period for a randomly chosen class $3$ defect
\DIFdelbegin \DIFdel{under the parameterization of the nearest class ($\bm{A}_{\RN{1}}$) , the relaxed Erlang ($\bm{A}_{\RN{2}}$) , and the up to 3 classes ahead ($\bm{A}_{\RN{4}}$), 
along with the corresponding eight-year }\DIFdelend \DIFaddbegin \DIFadd{comparing the standard MJP with global regression coefficients (\cref{eq:trans_rates_covariates}) with the state-dependent regression setting (\cref{eq:trans_rates_covariates2}) as well as the mixture MJP (\cref{sec:mixture}). The generator parameterization is held fixed ($\bm{A}_{\RN{3}}$). To the right, the corresponding ten-year }\DIFaddend transition probability matrices \DIFaddbegin \DIFadd{(TPMs) are displayed}\DIFaddend .

\DIFaddbegin \input{transient_dists}

\DIFaddend The first row of each \DIFdelbegin \DIFdel{transition matrix }\DIFdelend \DIFaddbegin \DIFadd{TPM }\DIFaddend corresponds to the transient distribution after \DIFdelbegin \DIFdel{eight years. 
...
This difference likely reflects the relatively low number of observed class $2B$ defects, causing $\bm{A}_{\RN{4}}$ to assign little probability to that stateover time}\DIFdelend \DIFaddbegin \DIFadd{ten years. 
The standard MJP places the majority of probability mass in the absorbing class $0$ after ten years (\cref{fig:trans_dist_1}), whereas the MJP with state-dependent regression assigns most probability at class $1$ (\cref{fig:trans_dist_2}), but with substantially less sharpness (i.e. probabilities are more equally spread across classes). The mixture MJP approaches a state distribution that tends to evolve more statically after two years (\cref{fig:trans_dist_3}) than its counterparts.
}

\paragraph{\DIFadd{Turning points in the evolution of class probabilities.}} \label{sec:turning_points}
\DIFadd{With upper triangular generators, the transient distribution of the standard MJP follows a simple principle: once a class probability starts to decrease, it will not increase again. This results in curves with at most one turning point, cf. \cref{fig:trans_dist_1}. 
...
Therefore, when the transient distribution is plotted against the observation interval, multiple turning points can occur, which is also evident in \cref{fig:trans_dist_2}. For instance, we see that the class $1$ probability increases rapidly during the first year, then it decreases slowly until year five, where it begins to increase again. Increasing the time horizon would reveal this class $1$ probability to finally approach zero, as the defect reaches the absorbing state}\DIFaddend .

\DIFdelbegin \DIFdel{$\bm{A}_{\RN{2}}$ (\Cref{fig:trans_dist_gerlang_relax}) assigns less probability mass }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{The implications of different model specifications.}}
\DIFadd{The models specifications provide different ways to represent the degradation mechanisms. The transient distribution of the standard MJP offers an interpretation of the modeled degradation which is easily conveyable to practitioners: 
...
%DIF > Because $\bm{A}_{\RN{1}}$ only allows jumps to neighboring classes, it assigns substantially more probability mass to class $2B$ (see \cref{fig:trans_dist_gerlang}) than $\bm{A}_{\RN{4}}$ (\cref{fig:trans_dist_tridiagonal}), which permits direct jumps to more severe categories without passing through all intermediate states.
%DIF > This difference likely reflects the relatively low number of observed class $2B$ defects, causing $\bm{A}_{\RN{4}}$ to assign little probability to that state over time.

%DIF > $\bm{A}_{\RN{2}}$ (\cref{fig:trans_dist_gerlang_relax}) assigns less probability mass to class $2B$ than $\bm{A}_{\RN{1}}$, but more than $\bm{A}_{\RN{4}}$. However, it differs notably from both in its treatment of the most severe classes: it assigns substantially more probability mass to class $1$ and correspondingly less to class $0$ after ten years.
%DIF > This shift stems from the core assumption of $\bm{A}_{\RN{2}}$ (\cref{eq:trans_rate_param}), where the degradation time between two observed states is modeled as the sum of intermediate sub-degradation processes.
%DIF > As a result, probability mass is systematically shifted toward intermediate states, leading to the observed leftward bias in the transition matrix compared to $\bm{A}_{\RN{1}}$ and $\bm{A}_{\RN{4}}$.





%DIF > While $\bm{A}_{\RN{1}}$ enforces gradual transitions between neighboring severity classes, it can still capture sudden defect propagation through rapid sequences of small jumps.Conversely, $\bm{A}_{\RN{4}}$ (as well as $\bm{A}_{\RN{3}}$ and $\bm{A}_{\RN{5}}$) allows direct jumps to more severe classes without passing through intermediates, making it well-suited for modeling abrupt failures caused by random shocks, such as those imposed by heavy rolling stock loads or external impacts. $\bm{A}_{\RN{2}}$ lies between these two extremes: it models large transitions as the cumulative effect of smaller degradation steps, combining gradual wear processes with the possibility of accelerated defect growth.

%DIF >  Note that the first row in the transition probability matrices corresponds to the transient distribution after eight years. As the generalized Erlang only models jumps to the neighboring class, it assigns substantially more probability mass to class $2B$ (see \cref{fig:trans_dist_gerlang}) than the free upper parameterization in \cref{fig:trans_dist_free_upper}, which is not restricted to visit $2B$ before it can move further into more severe categories. Most likely, the relatively low number of $2B$ observations is the reason why the free upper parameterization from \cref{eq:generator_free_upper} assigns low probability to that state over the eight year period. The reparameterized upper in \cref{fig:trans_dist_param_upper} assigns less probability mass to category $2B$ than the generalized Erlang, but more than the free upper. However, the reparameterized upper from \cref{eq:trans_rate_param} differs notably from the two other parameterizations wrt. category $1$ and $0$ where it assigns significantly more probability mass to class $1$ than the generalized Erlang and the free upper. Most of this probability mass comes from class $0$ which has substantially lower probability mass after eight years than in the cases of the generalized Erlang and the free upper. This is caused by the core assumption of the reparameterized upper in \cref{eq:trans_rate_param}, where the degradation time between two observed states is the sum of all intermediate sub-degradation processes. In general, the assumption will force more probability mass to the left in the transition probability matrix, which indeed is the case in \cref{fig:trans_dist_param_upper} compared to \cref{fig:trans_dist_gerlang} or \cref{fig:trans_dist_free_upper}.
\DIFaddend % Although, the free upper parameterization has the highest prediction accuracy, the reparameterized upper has its merits. The parameter spaces of the free upper increases quadratically with the number of categories in the state space, while it increases linearly for the reparameterized upper. For the fairly small state space considered in this present study ($m = 5$), model estimation appears robust. However, for larger state spaces it might be more problematic to robustly determine all parameters using the free upper parameterization. In that case, the reparameterized upper could offer an alternative with as few parameters as the generalized Erlang but with more flexibility for jumps in the Markov chain.
...
\DIFdel{Forecast diagnostics of reliability }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Transition residuals}} \label{sec:transition_patterns}

\DIFadd{To compare the model-implied transitions with the empirical ones, define the elements in the transition count matrix
$
...
Then, the transition residuals for a model can be calculated as
  ...
  R_{i,j} = \left(  N_{i,j}^{\text{obs.}} - \widehat{  N}_{i,j} \right) / \sum_{j}   N_{i,j}^{\text{obs.}},
$$ \DIFadd{for $i,j \in \mathcal J$.
...
\begin{table}[h]
    \begin{subtable}[h]{0.45\textwidth}
        \centering
          ...
          \caption{\DIFaddFL{Transition matrix, $ N_{i,j}^{\text{obs.}} / \sum_{j}  N_{i,j}^{\text{obs.}}$.}}
    ...
    \begin{subtable}[h]{0.45\textwidth}
       ...
       \input{expected_transition_bidiagonal_cov1_exp_exp_all_no_warp}
        \caption{\DIFaddFL{$\bm R$ given $\bm{A}_{\RN{3}}$ with global regression (\cref{eq:trans_rates_covariates}).}}
     ...
     \end{subtable}
      \begin{subtable}[h]{0.45\textwidth} 
        ...
        \caption{\DIFaddFL{$\bm R$ given $\bm{A}_{\RN{3}}$ with state-dependent regression (\cref{eq:trans_rates_covariates2}).}}
 ...
 \input{expected_transition_bidiagonal_cov1_exp_exp_all_no_warp_mixture4}
        \caption{\DIFaddFL{$\bm R$ given mixtures of $\bm{A}_{\RN{3}}$ (\cref{sec:mixture} with $G = 4$) and global regression (\cref{eq:trans_rates_covariates}).}}
        \label{tab:expected_transition_bidiagonal_cov1_exp_exp_all_no_warp_mixture4}
     \end{subtable} 
     \caption{\DIFaddFL{Transition residuals comparing model-implied and empirical transitions.}}
     \label{tab:transition_patterns}
\end{table}


\DIFadd{As seen from the proportions in \cref{tab:transition_matrix}, there is a substantial amount of transitions appearing over more than one class ahead. Overall, the $\bm{A}_{\RN{3}}$ with state-dependent covariates (\cref{tab:expected_transition_bidiagonal_cov2_exp_exp_all_no_warp}) improves the residuals compared to the setting with global regression coefficients (\cref{tab:expected_transition_bidiagonal_cov1_exp_exp_all_no_warp}), as indicated by a lower L1-norm (0.385 vs 0.327). Both settings have their largest residuals in the $1 \rightarrow 0$ transition }\DIFaddend and \DIFaddbegin \DIFadd{overpredict the probability of reaching the absorbing state (class $0$). Clearly, the mixture model matches the empirical transition frequencies more closely (\cref{tab:expected_transition_bidiagonal_cov1_exp_exp_all_no_warp_mixture4}), with an L1-norm of approx. 0.020, which is expected given its flexibility in adapting to data. In contrast, the single-generator MJP settings are more constrained by the assumed degradation mechanism, while the observed transition data may be influenced by practices and policies that prevent many defects from progressing to rail breaks once they reach a safety-critical level (e.g., class $1$).
}

\DIFadd{Appendix \ref{sec:transition_L1} provides a full summary of the L1-norms of the $\bm R$'s from all outlined predictors.
}




\subsubsection{\DIFadd{The impact of the covariates: An example}} \label{sec:risk_curves}
\DIFadd{In practice, infrastructure managers could be interested in quantifying the impact of the covariates on the risk of obtaining a critical defect. We here provide an example of how this can be done within the existing model framework.
...
\begin{subfigure}{.33\textwidth}
  ...
  \caption{\DIFaddFL{Tonnage}}
  \label{fig:crit_curves_1}
\end{subfigure}%DIF > 
\begin{subfigure}{.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{crit_curves_5}
  \caption{\DIFaddFL{Curvature}}
  \label{fig:crit_curves_5}
\end{subfigure}
\begin{subfigure}{.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{crit_curves_3}
  \caption{\DIFaddFL{Rail weight}}
  \label{fig:crit_curves_3}
\end{subfigure}
\caption{\DIFaddFL{Risk curves $r^{(s)}(t)$ under varying impacts of covariates using an MJP with $\bm{A}_{\RN{3}}$ generator and state-dependent regression coefficients (\cref{eq:trans_rates_covariates2}).}}
\label{fig:critical_curves}
\end{figure}

\DIFadd{From \cref{fig:crit_curves_1} and \cref{fig:crit_curves_5}, we see that the model associates high tonnage and curvature values with higher risk of reaching $\mathcal J_{\text{critical}}$, in line with existing domain knowledge that increasing intensity of traffic increases defect progression. \cref{fig:crit_curves_3} indicates that bigger rail profiles lower the risk of a defect progression into a critical class.
...
\subsection{\DIFadd{Calibration, }\DIFaddend discrimination\DIFaddbegin \DIFadd{, and sharpness}\DIFaddend } \label{sec:reliability_analysis}
\DIFdelbegin \DIFdel{So far, forecast evaluation has relied on }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend proper scoring rules \DIFdelbegin \DIFdel{. These showed that }\DIFdelend \DIFaddbegin \DIFadd{in \cref{sec:prediction_error} showed that the }\DIFaddend flexible MJP parameterizations outperform \DIFdelbegin \DIFdel{reference predictors}\DIFdelend \DIFaddbegin \DIFadd{the reference predictors (most substantially the mixture MJPs)}\DIFaddend , and that modifying sojourn time estimates, exogenous information, and averaging over all predictors in an ensemble \DIFdelbegin \DIFdel{further }\DIFdelend improve accuracy.
However, such scores reduce forecast quality to single summary values, and therefore cannot reveal the specific sources of deficiencies \citep{hamill1997,Dimitriadis_2024}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To }\DIFdelend \DIFaddbegin \DIFadd{So far, we supplemented this evaluation by inspecting transition residuals (\cref{sec:transition_patterns}), but to }\DIFaddend provide a more detailed analysis, \DIFdelbegin \DIFdel{we generate }\DIFdelend multicategorical reliability diagrams \DIFaddbegin \DIFadd{are generated }\DIFaddend to assess forecast calibration, and checkerboard plots to evaluate discrimination ability.\DIFaddbegin \footnote{\DIFadd{Our GitHub repository (}\url{https://github.com/dobbeltgaard/MarkovJump.git}\DIFadd{) provides \texttt{R} code to generate these diagnostic plots.}} \DIFadd{Also, the sharpness of the predictors will be assessed using an entropy measure.
}

\subsubsection{\DIFadd{Multicategory reliability diagrams}}
\DIFaddend We follow the methodology proposed by \citet{hamill1997}, extending the framework of reliability diagrams initially introduced for binary forecasts by \citet{wilks1995a}.
...
\begin{enumerate}    
    ...
    \DIFdelbegin \DIFdel{We use $K = 10$.
    }\DIFdelend \item For each forecast, compute the cumulative probabilities across the $m$ categories. For each quantile $q_k$, determine the forecasted category as the smallest category where the cumulative probability exceeds or equals $q_k$. This converts the probabilistic forecasts into categories at each quantile.
...
\DIFdelbegin \DIFdel{Our GitHub repository}\footnote{%DIFDELCMD < \url{https://github.com/dobbeltgaard/MarkovJump.git}%%%
} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{provides \texttt{R} code to generate these diagnostic plots .
}%DIFDELCMD < \input{reliability_figs} 
%DIFDELCMD < %%%
\DIFdelend 

\DIFaddbegin \subsubsection{\DIFadd{Entropy measure}}
\DIFadd{To assess forecast sharpness, we define for a predicted distribution $\widehat{\bm\pi}^{(s)}$ a normalized entropy measure by
}\begin{align}
...
\end{align}
\DIFadd{This quantifies the average level of uncertainty that a prediction assigns over the categories with $H \in [0,1]$, so that the sharper a forecast is, the lower $H$ becomes.
}

\subsubsection{\DIFadd{Summary of calibration, discrimination, and sharpness}}
\DIFadd{Based on the out-of-fold predictions from the CV procedure in \cref{sec:prediction_error}, the multicategory reliability diagrams with checkerboard plots are generated, so each defect contributes once. Further, we report mean predictive entropy as a measure of sharpness, computed for each held-out forecast and averaged (a full list is provided in Appendix \ref{sec:sharpness}).
}

\DIFaddend To illustrate, we focus on four \DIFdelbegin \DIFdel{representative predictors: 
(}\DIFdelend \DIFaddbegin \DIFadd{predictors: 
}\DIFaddend 1) the uniform predictor\DIFdelbegin \DIFdel{, providing a simplistic lower bound; 
(}\DIFdelend \DIFaddbegin \DIFadd{; 
}\DIFaddend 2) the logistic CLM with exogenous information\DIFdelbegin \DIFdel{, representing the strongest static baseline; 
(}\DIFdelend \DIFaddbegin \DIFadd{; 
}\DIFaddend 3) the MJP with parameterization $\bm{A}_{\RN{3}}$ including exogenous information\DIFdelbegin \DIFdel{and the modified sojourn time estimates, as a representative dynamic model;
and 
(}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend 4) \DIFdelbegin \DIFdel{the ensemble forecast, combining all predictors. 
Results are shown in \Cref{fig:reliability_diagrams}. }\DIFdelend \DIFaddbegin \DIFadd{and the MJP mixture using the $\bm{A}_{\RN{3}}$ parameterization including exogenous information. 
}\DIFaddend Bootstrapping was performed with 200 resamples, and 10\% and 90\% confidence intervals are reported. \DIFaddbegin \DIFadd{We use $K = 10$ quantiles ($0.05, 0.15, \dots, 0.95$). Results are shown in \cref{fig:reliability_diagrams}.
...
%DIF < For brevity, we evaluate four predictors from \cref{tab:forecast_error_tab};
%DIF < (1) the uniform distribution predictor (naïve, nonparametric),
%DIF < (2) the cumulative link model with logistic link and exogenous information,
%DIF < (3) the Markov jump process (using $\bm{A}_{\RN{3}}$) with exogenous information and temporal warping, as well as
%DIF < (4) the ensemble forecast combining all predictors.
%DIF < Results are shown in \Cref{fig:reliability_diagrams}. 
\DIFaddbegin \input{reliability_figs}
\DIFaddend 


The uniform distribution predictor (\DIFdelbegin \DIFdel{\Cref{fig:reliability_diag_uniform}}\DIFdelend \DIFaddbegin \DIFadd{\cref{fig:reliability_diag_uniform}}\DIFaddend ) substantially overestimates the fraction of observed categories across all quantiles, indicating \DIFdelbegin \DIFdel{pronounced }\DIFdelend \DIFaddbegin \DIFadd{high }\DIFaddend miscalibration.
Its average category error is also the highest among the four predictors, consistent with its poor performance on aggregate \DIFdelbegin \DIFdel{error scores in \cref{tab:forecast_error_tab}. }\DIFdelend \DIFaddbegin \DIFadd{scores in \cref{tab:prediction_scores}. Its average normalized entropy is $0.8093$, which is expected since a uniform predictor is preferenceless, meaning it has minimal sharpness.}\footnote{\DIFadd{Remark the only reason it does not attain the maximum value of $1$ for the average entropy is that \cref{eq:uniform_predictor} only spreads out probability mass to the states that are reachable given the initial observation $y_1^{(s)}$ (if the support size is $r<m$, the maximum normalized entropy becomes $\log(r)/\log(m) < 1$).}}
\DIFaddend 

The cumulative link model with logistic link (\DIFdelbegin \DIFdel{\Cref{fig:reliability_diag_olr_cov}) shows notable }\DIFdelend \DIFaddbegin \DIFadd{\cref{fig:reliability_diag_olr_cov}) shows }\DIFaddend improvement in calibration relative to the uniform distribution predictor.
\DIFdelbegin \DIFdel{However, its }\DIFdelend \DIFaddbegin \DIFadd{Its }\DIFaddend discrimination ability remains modest, as indicated by large average category errors and checkerboard plots with heavily populated bins far from the zero-error line. \DIFaddbegin \DIFadd{Average entropy is large ($0.7416$).
}\DIFaddend 

The \DIFdelbegin \DIFdel{Markov jump process (\Cref{fig:reliability_diag_bidiagonal_TRUE_exp_exp_all_warp}}\DIFdelend \DIFaddbegin \DIFadd{MJP (\cref{fig:reliability_diag_bidiagonal_cov1_exp_exp_all_no_warp}}\DIFaddend ) achieves the lowest mean category error among the single predictors, and its checkerboard plot shows concentration of errors near the zero-category line, indicating good discrimination ability.
Its reliability curve \DIFdelbegin \DIFdel{closely follows the ideal diagonal, suggesting well-calibrated forecasts.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The ensemble forecast (\Cref{fig:reliability_diag_ensemble_all}) yields slight improvements in calibration at higher quantiles but slight worsening at lower quantiles.
This behavior can be explained by the interaction of the individual models: while the cumulative link model tends to overpredict severity at higher quantiles , the MJP tends to underpredict, resulting in partial cancellation}\DIFdelend \DIFaddbegin \DIFadd{indicates that it tends to underpredict severity, most noticeably at higher quantiles.
The mixture MJP (\cref{fig:reliability_diag_bidiagonal_cov1_exp_exp_all_no_warp_mixture4}) has larger mean category error, but better calibrated forecasts at the higher quantiles (cf. \cref{sec:transition_patterns}, where it better matches empirical transition frequencies)}\DIFaddend . However, \DIFdelbegin \DIFdel{at lower quantiles , both models tend to underpredict, leading to no corrective effect.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The ensemble forecast appears somewhat less sharp than the MJP alone, as reflected in higher mean category error and more dispersed errors in the checkerboard plots.
Nevertheless, the differences remain relatively minor, and in all cases, the diagnostic plots confirm the improvements in predictive performance indicated by the aggregate scores}\DIFdelend \DIFaddbegin \DIFadd{it has weakened discrimination ability at the higher quantiles (see the darker bins away from zero at the two last bins in the checkerboard), indicating a trade-off between calibration and discrimination ability.
The entropy of the single generator MJP is $0.5249$ compared to $0.5857$ for the mixture MJP, which suggests that the mixture MJP is more conservative in how it distributes the probability mass (also reflected in slower evolution of the rail break risk in \cref{sec:results_transient_dist_markov}). As the mixture MJP was the predictor with the highest predictive accuracy in terms of RPS and log score, it suggests that a loss of sharpness can be beneficial if it improves calibration enough}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Proposition for use case} \label{sec:proposition_for_use}
From the perspective of an infrastructure manager, probabilistic forecasts in isolation have limited operational value. 
\DIFdelbegin \DIFdel{To turn them into actionable strategies}\DIFdelend \DIFaddbegin \DIFadd{For better network operation}\DIFaddend , forecasts must be embedded in decision frameworks that allocate \DIFdelbegin \DIFdel{inspection and maintenance }\DIFdelend resources under uncertainty.\DIFaddbegin \footnote{\DIFadd{Discussions with BaneNOR and Banedanmark (the infrastructure managers responsible for most of the Norwegian and Danish networks) highlighted that connecting predictions to actionable maintenance strategies is essential for realizing the value of predictive models.}}
%DIF > One possible implementation pathway could be threshold-based decisions under a fixed budget: for each defect, generate forecasts over a six-month horizon and allocate resources to the $n$ defects with the highest probability of reaching a critical state, with $n$ bounded by the budget divided by the average per-defect action cost. Such a heuristic is unlikely to lead to near-optimal network operation, because it ignores the value of information from inspection and the timing of decisions over multiple horizons.
\DIFadd{In the following, we outline and solve a stochastic program that connects probabilistic forecasts to sequential inspection and maintenance decisions over multiple stages.
}\DIFaddend 

\DIFdelbegin \DIFdel{Discussions with BaneNOR and Banedanmark (the infrastructure managers responsible for most of the Norwegian and Danish networks) highlighted that connecting predictions to actionable maintenance strategies is essential for realizing the value of predictive models.
In practice, this requires translating forecast distributions of defect severity into decisions thatbalance safety, cost, and resource constraints. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{One possible implementation pathway is a threshold-based allocation under a fixed budget: for each defect $s\in\mathcal{S}$, generate forecasts over a six-month horizon and allocate resources to the $n$ defects with the highest probability of reaching a critical “rail-break” state, with $n$ bounded by the budget divided by the average per-defect action cost.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Alternatively, the predictive distributions can be embedded into a stochastic optimization problem.
Since we consider a discrete stochastic process, it can be represented by a finite set of scenarios over a time partition $\widehat{T}=(T_1,\dots,T_n)$, where $T_n$ isthe decision horizon.
Given active defects }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{A stochastic program}}
\DIFadd{Here, we formulate an optimization model that, based on the predictions, minimizes expected cost over a time horizon of $T_n$ related to active defects in a set }\DIFaddend $\mathcal{S}$ \DIFdelbegin \DIFdel{and a }\DIFdelend \DIFaddbegin \DIFadd{across a railway network with a }\DIFaddend rail line mapping \DIFdelbegin \DIFdel{$\ell:\mathcal{S}\to\{\text{rail lines}\}$, let $\widehat{\bm\pi}^{(s)}_j(t)=\mathds{P}(X^{(s)}(t)=j)$ be the predicted probability for class $j\in\mathcal{J}$ at time $t\in\widehat{T}$ (cf.
\cref{eq:transient_dist}). Define
}%DIFDELCMD < \[
%DIFDELCMD < y^{(s)}(t)\in\{0,1\}\ \ \text{(inspection at $t$)},\qquad
%DIFDELCMD < z^{(s)}(t)\in\{0,1\}\ \ \text{(maintenance at $t$)}.
%DIFDELCMD < \]%%%
\DIFdelend \DIFaddbegin \DIFadd{$\ell:\mathcal{S}\to\{\text{rail lines}\}$.
}


\paragraph{\DIFadd{Decision variables and available information.}}
\DIFadd{We will consider maintenance and inspection planning on a time grid $\widehat T=(T_1,\dots,T_n)$.
At these time points (stages), we define decision variables 
$y^{(s)}(t)\in\{0,1\}$ for inspection and $z^{(s)}(t)\in\{0,1\}$ for maintenance at time $t \in \widehat T$ for defect $s\in\mathcal S$.
%DIF > $$
%DIF > y^{(s)}(t)\in\{0,1\}\quad\text{(inspection at \(T_t\))}, \qquad %\text{and }
%DIF > z^{(s)}(t)\in\{0,1\}\quad\text{(maintenance at \(T_t\))},
%DIF > $$
%DIF > for $t = 1, \dots, n$.
...
\end{cases}
$$\DIFaddend 
\DIFdelbegin \DIFdel{Let $q^{(s)}_{\mathrm{break}}(t)=\sum_{j\in\mathcal{J}_{\mathrm{crit}}}\widehat{\bm\pi}^{(s)}_j(t)$ be the predicted probability of being in a critical class set $\mathcal{J}_{\mathrm{crit}}$ by $t$.
Let $c_{\mathrm{ins}}^{\ell}$ be rail line specific inspection cost , $c_{\mathrm{maint}}^{\ell}(j)$ the (severity-dependent) maintenance cost, and $c_{\mathrm{fail}}^{\ell}$ the monetized expected loss }\DIFdelend \DIFaddbegin \DIFadd{Note that at each stage $t$, the predicted distributions $\widehat{\bm \pi}^{(s)}(t \mid \mathcal F(t))$ are computed given the information available in $\mathcal F(t)$, and so they are $\mathcal F(t)$-measurable.
}

\DIFadd{We let the indicator \(a^{(s)}(t)\in\{0,1\}\) express that defect \(s\) is still active, which means not maintained yet. We define \(\chi^{(s)}(t)\in\{0,1\}\) as an indicator that defect \(s\) is still uninspected. Subsequently, $a^{(s)}(t)$ and $\chi^{(s)}(t)$ are auxiliary variables that will help us write up the optimization program compactly.
}


\paragraph{\DIFadd{Costs and critical class risks.}}
\DIFadd{Let the inspection cost at line $\ell$ be denoted by $c_{\text{ins}}^{\ell(s)}$, and $c_{\text{fail}}^{\ell(s)}$ the estimated costs }\DIFaddend of a rail break \DIFdelbegin \DIFdel{on line $\ell$ (e.g. combining derailment risk and societal cost).
Let $B(t)$ denote the per-period budget (or a total budget $B$).
The objective (considering risk, repairs, and inspection) }\DIFdelend \DIFaddbegin \DIFadd{for $s \in \mathcal S$.
...
$ where $c^{\ell(s)}_{\text{maint}}(j)$ is the maintenance cost for severity class $j$.
}

\DIFadd{Further, we define a critical-state probability, i.e., 
$
...
\paragraph{\DIFadd{The objective function and constraints.}}
\DIFadd{The multistage stochastic program }\DIFaddend can then be written as \DIFdelbegin \DIFdel{the expected cost model
}%DIFDELCMD < \begin{align}
%DIFDELCMD < \min_{y,z}\ \ 
%DIFDELCMD < &\sum_{t\in\widehat{T}}\sum_{s\in\mathcal{S}}
%DIFDELCMD < \Big[
%DIFDELCMD < c_{\mathrm{ins}}^{\ell(s)}\,y^{(s)}(t)
%DIFDELCMD < + z^{(s)}(t)\,\sum_{j\in\mathcal{J}} c_{\mathrm{maint}}^{\ell(s)}(j)\,\widehat{\bm\pi}^{(s)}_{j}(t)
%DIFDELCMD < \Big]
%DIFDELCMD < + \sum_{t\in\widehat{T}}\sum_{s\in\mathcal{S}} 
%DIFDELCMD < c_{\mathrm{fail}}^{\ell(s)}\, q^{(s)}_{\mathrm{break}}(t)\,\Bigl(1-\!\!\sum_{\tau\le t} z^{(s)}(\tau)\Bigr)
%DIFDELCMD < \label{eq:obj-expected}\\[-0.25em]
%DIFDELCMD < \text{s.t.}\ \ 
%DIFDELCMD < &\sum_{s\in\mathcal{S}}\!\Big(c_{\mathrm{ins}}^{\ell(s)}\,y^{(s)}(t) 
%DIFDELCMD < + z^{(s)}(t)\,\sum_{j\in\mathcal{J}} c_{\mathrm{maint}}^{\ell(s)}(j)\,\widehat{\bm\pi}^{(s)}_{j}(t)\Big)\ \le\ B(t), \quad \forall t\in\widehat{T}, \label{eq:budget}\\
%DIFDELCMD < &y^{(s)}(t),z^{(s)}(t)\in\{0,1\}, \qquad
%DIFDELCMD < y^{(s)}(t)+z^{(s)}(t)\le 1, \quad \forall s\in\mathcal{S},\ \forall t\in\widehat{T}. \label{eq:binary}
%DIFDELCMD < \end{align}%%%
\DIFdelend \DIFaddbegin \DIFadd{a minimization of the expected costs, that is,
}\begin{align}
\min_{y,z,a,\chi}\quad
\mathbb E\!\left[
...
+ \bar c^{(s)}(T_k) z^{(s)}(T_k)
+ c_{\mathrm{fail}}^{\ell(s)}\,q^{(s)}(T_k)\bigl(a^{(s)}(T_k)-z^{(s)}(T_k)\bigr)
\Bigr)
\right]
\label{eq:general_textbook_obj}
\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{The factor $\bigl(1-\sum_{\tau\le t} z^{(s)}(\tau)\bigr)$ switches off break losses once maintenance is performed (assuming maintenance returns the defect to a safe state). 
The constraints enforce the budget }\DIFdelend \DIFaddbegin \DIFadd{subject to
}\begin{align}
\sum_{s\in\mathcal S}
\Bigl(
...
\Bigr)
&\le B(T_k)
\qquad \text{a.s.},\quad k = 1,\dots,n,
\label{eq:general_textbook_budget}\\
y^{(s)}(T_k)+z^{(s)}(T_k)
&\le a^{(s)}(T_k)
\qquad \text{a.s.},\quad s\in\mathcal S,\  k = 1,\dots,n,
\label{eq:general_textbook_exclusive}\\
y^{(s)}(T_k)
&\le \chi^{(s)}(T_k)
\qquad \text{a.s.},\quad s\in\mathcal S,\  k = 1,\dots,n,
\label{eq:general_textbook_inspection}\\
a^{(s)}(T_1)&=1,\quad \chi^{(s)}(T_1)=1,
\qquad s\in\mathcal S,
\label{eq:general_textbook_initial}\\
a^{(s)}(T_{k+1})&=a^{(s)}(T_k)-z^{(s)}(T_k)
\qquad \text{a.s.},\quad s\in\mathcal S,\ k=1,\dots,n-1,
\label{eq:general_textbook_activity}\\
\chi^{(s)}(T_{k+1})&=\chi^{(s)}(T_k)-y^{(s)}(T_k)
\qquad \text{a.s.},\quad s\in\mathcal S,\ k=1,\dots,n-1.
\label{eq:general_textbook_uninspected}
\end{align}
%DIF > All decision variables and auxiliary state variables are required to be \(\mathcal F_t\)-measurable at stage \(t\). 
\DIFadd{The objective in \cref{eq:general_textbook_obj} combines inspection cost, expected maintenance cost, and expected failure cost. 
%DIF > Note that $q^{(s)}(t)$ lumps the forecast distribution into a stage-wise critical risk that enters as expected failure exposure for active defects. 
\cref{eq:general_textbook_budget}--\cref{eq:general_textbook_inspection} ensure budget feasibility }\DIFaddend and at most one action per \DIFdelbegin \DIFdel{defect and period.
Extensions can model information gains from inspection (scenario updates)}\DIFdelend \DIFaddbegin \DIFadd{active defect and stage.
\cref{eq:general_textbook_initial}--\cref{eq:general_textbook_uninspected} ensure that the auxiliary variable are updated, so that maintenance removes a defect from the future decision stage and inspections update the information available for later decisions.}\footnote{\DIFadd{If an event has probability 1, then we say that this event occurs ``almost surely'' (a.s.) \citep[Chapter 3]{thygesen2023a}.}}

\subsubsection{\DIFadd{Solving a stochastic program: An example}}
\DIFadd{To solve a stochastic program like \cref{eq:general_textbook_obj}--\cref{eq:general_textbook_uninspected}, uncertainty is typically represented by generating scenarios structured in a decision tree.
...
Because the information available to the decision-maker depends on what is revealed by inspections, the optimization problem is subject to \textit{endogeneity}}\DIFaddend , 
\DIFdelbegin \DIFdel{crew logistics, or multi-horizon planning.
Beyond the expected-cost objective, there are many variants, e.g. robust programs minimizing with respect to worst case scenarios}\DIFdelend %DIF > the uncertainty is \textit{endogenous}, 
\DIFaddbegin \DIFadd{meaning that the filtration $\mathcal F(t)$ depends on the inspection decisions $y^{(s)}(\tau)$ for $\tau\le t$, which increases the number of information branches.
...
\paragraph{\DIFadd{Numerical solution.}}
\DIFadd{First, for $10$ selected defects we generate Monte Carlo scenarios on a three-stage grid $(T_1,T_2,T_3)$ with known severity classes at $T_0$}\DIFaddend .\DIFaddbegin \footnote{\DIFadd{First-stage decisions will therefore be made at $T_1$.}}
\DIFadd{This is done over a $1.5$ year horizon using transition probabilities from a fitted MJP with $\bm{A}_{\RN{3}}$-generator and regression (\cref{eq:trans_rates_covariates}). 
...
\centering
\caption{\DIFaddFL{First-stage decisions in the obtained policy.}} \label{tab:decisions}
\begin{tabular}{clcl}
\toprule
\DIFaddFL{Defect }& \DIFaddFL{Class at $T_0$ }& \DIFaddFL{\(q_{\mathrm{break}}(T_1)\) }& \DIFaddFL{Stage-1 decision }\\
\midrule
\DIFaddFL{1 }& \DIFaddFL{Class $3$ }& \DIFaddFL{0.0346 }& \DIFaddFL{Inspect }\\
\DIFaddFL{2 }& \DIFaddFL{Class $2A$ }& \DIFaddFL{0.1564 }& \DIFaddFL{Maintain }\\
\DIFaddFL{3 }& \DIFaddFL{Class $3$ }& \DIFaddFL{0.0285 }& \DIFaddFL{Inspect }\\
\DIFaddFL{4 }& \DIFaddFL{Class $2A$ }& \DIFaddFL{0.0677 }& \DIFaddFL{Wait }\\
\DIFaddFL{5 }&\DIFaddFL{Class $3$ }& \DIFaddFL{0.0165 }& \DIFaddFL{Inspect }\\
\DIFaddFL{6 }& \DIFaddFL{Class $3$ }& \DIFaddFL{0.0286 }& \DIFaddFL{Wait }\\
\DIFaddFL{7 }& \DIFaddFL{Class $2A$ }& \DIFaddFL{0.0912 }& \DIFaddFL{Wait }\\
\DIFaddFL{8 }& \DIFaddFL{Class $2B$ }& \DIFaddFL{0.0747 }& \DIFaddFL{Maintain }\\
\DIFaddFL{9 }& \DIFaddFL{Class $2A$ }& \DIFaddFL{0.0993 }& \DIFaddFL{Maintain }\\
\DIFaddFL{10 }& \DIFaddFL{Class $2A$ }& \DIFaddFL{0.1182 }& \DIFaddFL{Maintain }\\
\bottomrule
\end{tabular}
\end{table}
\DIFadd{As seen, the defects with lower severity are either assigned an inspection or no action, while the more progressed defects are assigned maintenance.
}\DIFaddend 


\DIFdelbegin \DIFdel{The focus of this study has been to develop probabilistic models for forecasting defect severity and evaluate their accuracy. The decision-making program sketched here is intended as a proposal for how such forecasts could be used in infrastructure manager operations, rather than as a prescriptive solution. }\DIFdelend \DIFaddbegin \DIFadd{Taking defects 5 and 6 as a small case study, we see that defect 5 is inspected while defect 6 is not, even though both start in class $3$. The normalized entropy (\cref{eq:entropy}) of the one-step forecast at $T_1$ is $0.48$ for defect 5 and $0.66$ for defect 6, so defect 6 has the less sharp forecast at the first stage. %DIF > However, the policy is not driven by forecast uncertainty alone: inspection is only valuable if it is likely to change downstream actions.
...
%DIF < 
%DIF < \subsection{Proposition for use case}
%DIF < From the perspective of an infrastructure manager, probabilistic forecasts in isolation have limited operational value. 
%DIF < To turn them into actionable strategies, forecasts must be embedded in decision frameworks that allocate inspection and maintenance resources under uncertainty.
%DIF < 
%DIF < Discussions with BaneNOR and Banedanmark (the infrastructure managers responsible for most of the Norwegian and Danish networks) highlighted that connecting predictions to actionable maintenance strategies is essential for realizing the value of predictive modeling. In practice, this requires translating forecast distributions of defect severity into decision-making that balances safety, cost, and resource constraints. 
%DIF < 
%DIF < One possible implementation pathway is to couple predictive distributions with a threshold-based decision framework under the assumption of a fixed maintenance budget. For instance, forecasts could be generated for each defect $s \in \mathcal{S}$ over a six-month horizon. Resources are then allocated to the $n$ defects with the highest probability of reaching the critical ``rail break'' state within that time, where $n$ is bounded by the budget divided by the average inspection or maintenance cost per defect.
%DIF < 
%DIF < Alternatively, the predictive distributions can be embedded into a stochastic optimization problem. 
%DIF < Since we consider a discrete stochastic process, it can be represented by a finite set of scenarios corresponding to all possible combinations of realizations over a finite partition of time, $\widehat{T} = (T_1, T_2, \dots, T_n)$, where $T_n$ denotes the chosen decision horizon of the infrastructure manager.
%DIF < Given a set of active defects, $\mathcal{S}$, on rail line $\ell(s)$ for $s \in \mathcal{S}$, and their predictive distributions $\bm \pi^{(s)}(t)$ (cf. \cref{eq:transient_dist}) for $t \in \widehat{T}$, we define the binary decision variables
%DIF < \[
%DIF < y^{(s)}(t)\in\{0,1\}\quad\text{(perform inspection)},\qquad
%DIF < z^{(s)}(t)\in\{0,1\}\quad\text{(perform maintenance)}.
%DIF < \]
%DIF < for defect $s$ at time $t$.
%DIF < Let 
%DIF < $q^{(s)}_{\text{break}}(t)=\sum_{j\in\mathcal{J}_{\text{crit}}}\bm \pi^{(s)}_j(t)$ denote the probability of being in a critical set of classes $\mathcal{J}_{\text{crit}}$ (e.g., rail-break risk) by time $t$.
%DIF < Let $c_{\text{ins}}^{\ell}$ and $c_{\text{maint}}^{\ell}(j)$ be line-specific inspection and maintenance costs (maintenance may depend on severity $j$ if heavier repairs are needed for worse cracks). 
%DIF < Let $c_{\text{fail}}^{\ell}$ be the monetized expected loss of rail break on line $\ell$, combining derailment risk and societal cost. 
%DIF < Define a severity-to-repair cost mapping $c_{\text{maint}}^{\ell}(j)$ that is low for small cracks (e.g., class~$3$/$2B$) and high once exceeding practical thresholds (requiring rail replacement).  Lastly, define a per-period budget $B(t)$ (or a single horizon budget $B$).
%DIF < The baseline objective (considering risk, repairs, and inspection) can then be written as the expected cost model
%DIF < \begin{align}
%DIF < \min_{y,z}\quad 
%DIF < &\sum_{t\in\widehat{T}}\sum_{s\in\mathcal{S}}
%DIF < \Bigg[
%DIF < \underbrace{c_{\text{ins}}^{\ell(s)}\,y^{(s)}(t)}_{\text{inspection}}
%DIF < +
%DIF < \underbrace{z^{(s)}(t)\,\sum_{j} c_{\text{maint}}^{\ell(s)}(j)\,p^{(s)}_{j}(t)}_{\text{expected maintenance cost at }t}
%DIF < \Bigg]
%DIF < \;+\;
%DIF < \underbrace{\sum_{t\in\widehat{T}}\sum_{s\in\mathcal{S}} 
%DIF < c_{\text{fail}}^{\ell(s)}\; q^{(s)}_{\text{break}}(t)\;\bigl(1-\sum_{\tau\le t} z^{(s)}(\tau)\bigr)}_{\text{expected break loss if not yet maintained}}
%DIF < \label{eq:obj-expected}
%DIF < \end{align}
%DIF < subject to
%DIF < \begin{align}
%DIF < &\sum_{s\in\mathcal{S}}\Big(c_{\text{ins}}^{\ell(s)}\,y^{(s)}(t) 
%DIF < + \, z^{(s)}(t)\,\sum_{j} c_{\text{maint}}^{\ell(s)}(j)\,p^{(s)}_{j}(t)\Big)\;\le\; B(t), 
%DIF < &&\forall t\in\widehat{T}, \label{eq:budget}\\
%DIF < &y^{(s)}(t)+z^{(s)}(t)\le 1, &&\forall s,\,t, \label{eq:mutual-excl}\\
%DIF < &z^{(s)}(t)\in\{0,1\},\; y^{(s)}(t)\in\{0,1\}, &&\forall s,\,t.
%DIF < \end{align}
%DIF < The constraints ensure the solution respects the budget constraint and that at most one action per defect and period can be performed.
%DIF < The factor $\bigl(1-\sum_{\tau\le t} z^{(s)}(\tau)\bigr)$ turns off break losses once maintenance is performed (we assume maintenance returns the defect to a safe state).
%DIF < Beyond the expected-cost objective, there are many variants, e.g. robust programs minimizing with respect to worst case scenarios.
%DIF < 
%DIF < 
%DIF > > entropy(t(c(1,0,0,0,0)) %*% TPMS[[3]]$TPM$dt_0.5)
%DIF > [1] 0.6570918
%DIF > > entropy(t(c(1,0,0,0,0)) %*% TPMS[[5]]$TPM$dt_0.5)
%DIF > [1] 0.4813692
%DIF > > entropy(t(c(1,0,0,0,0)) %*% TPMS[[6]]$TPM$dt_0.5)
%DIF > [1] 0.6575578

%DIF > questions for marcel: 
%DIF > root class or initial distribution?
\DIFaddbegin 


\DIFaddend \section{\DIFdelbegin \DIFdel{Conclusion}\DIFdelend \DIFaddbegin \DIFadd{Discussion and conclusions}\DIFaddend } \label{sec:conclusion}
We have proposed modeling partially observed rail defect severity trajectories using a Markov jump process (MJP) estimated via optimum score estimation \DIFdelbegin \DIFdel{, aligning training }\DIFdelend \DIFaddbegin \DIFadd{to align estimation }\DIFaddend with the chosen forecast evaluation metric. The framework accommodates \DIFdelbegin \DIFdel{covariate-dependent transition rates }\DIFdelend \DIFaddbegin \DIFadd{covariate-dependence }\DIFaddend (tonnage, line speed, curvature, grade, rail profile) \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{through a global regression and one with state-dependent regression coefficients. The study also considers }\DIFaddend multiple generator parameterizations, \DIFdelbegin \DIFdel{and }\DIFdelend a modification of sojourn time estimates\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and an extension to a mixture model.
...
Through systematic forecast evaluation with proper scoring rules, we benchmarked MJPs against reference predictors, including a uniform predictor\DIFaddbegin \DIFadd{, a discrete-time Markov chain, }\DIFaddend and cumulative link models with logistic, probit, and complementary log-log links. Results from \DIFaddbegin \DIFadd{$k$-fold }\DIFaddend cross-validation show that MJPs consistently outperform all reference predictors, achieving higher accuracy across both log score and ranked probability score\DIFdelbegin \DIFdel{. Forecast diagnostics confirm that MJPs provide better calibration and discrimination ability, while ensemble forecasts further enhance accuracy}\DIFdelend \DIFaddbegin \DIFadd{; however, estimation of prediction error through $k$-fold cross-validation can be biased \citep{fushiki2011a}}\DIFaddend .

The choice of generator parameterization \DIFdelbegin \DIFdel{strongly }\DIFdelend affects predictive performance: more flexible structures \DIFdelbegin \DIFdel{, which allow larger or skip transitions, }\DIFdelend yield higher accuracy \DIFaddbegin \DIFadd{in our experiments}\DIFaddend . Modifying sojourn time estimates improves performance across all parameterizations\DIFdelbegin \DIFdel{, and including exogenous information further enhances predictive accuracy , reflecting }\DIFdelend \DIFaddbegin \DIFadd{. 
...
\DIFadd{Including exogenous information results in increased predictive accuracy across all regression-based predictors (which strongly suggests that accounting for }\DIFaddend the heterogeneity of local track conditions \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{is beneficial). We see further improvements from introducing state-dependency in the regression, which may indicate complex dependencies at play between transition behavior and covariates. This study's investigation of which covariates are the most important is somewhat limited, as it builds upon the work of \citet{bisgaard2024A}, and most importantly, due to data availability. We encourage future work to provide insight into the statistical associations between crack dynamics and other track related measurements.
}\DIFaddend 

Finally, we \DIFdelbegin \DIFdel{sketched }\DIFdelend \DIFaddbegin \DIFadd{showed }\DIFaddend how predictive distributions can be embedded in a stochastic optimization program to allocate inspection and maintenance actions under budget constraints\DIFdelbegin \DIFdel{. This illustrates }\DIFdelend \DIFaddbegin \DIFadd{, minimizing expected costs. 
...
The program is implemented and solved for a smaller amount of defects, showing }\DIFaddend how probabilistic forecasts can be connected to actionable decision-making for infrastructure managers\DIFdelbegin \DIFdel{, bridging }\DIFdelend \DIFaddbegin \DIFadd{. The authors are hopeful that this could help bridge }\DIFaddend the gap between \DIFdelbegin \DIFdel{statistical modeling and practical }\DIFdelend \DIFaddbegin \DIFadd{predictive modeling and }\DIFaddend maintenance planning.  \DIFaddbegin \DIFadd{Future research could focus on developing more sophisticated decision policies, for example by solving the proposed multistage program in a rolling-horizon setting. Alternative policy classes, such as those described in \citet{powell2018a}, may also be considered.
}\DIFaddend 



\DIFdelbegin \DIFdel{The proposed methodology thus }\DIFdelend \DIFaddbegin \DIFadd{This study }\DIFaddend contributes both theoretically and practically: it advances \DIFdelbegin \DIFdel{CTMC-based }\DIFdelend \DIFaddbegin \DIFadd{Markov chain-based }\DIFaddend degradation modeling by introducing \DIFdelbegin \DIFdel{a sojourn time modification heuristic }\DIFdelend \DIFaddbegin \DIFadd{various model specifications }\DIFaddend and scoring-rule-consistent estimation, and it demonstrates how probabilistic forecasts can support infrastructure management. Beyond railways, the framework is broadly applicable to multicategorical forecasting problems in other domains where degradation processes evolve over ordinal state spaces and inspection intervals \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend irregular.
...
\section*{Software}
Model implementation is \DIFaddbegin \DIFadd{primarily }\DIFaddend carried out in \texttt{R} version 4.3.1 \citep{R_stats} and \texttt{C++} integrated with \texttt{R} via \texttt{Rcpp} \citep{Rcpp}, \texttt{Eigen} \citep{eigenweb}, and \texttt{TMB} \citep{TMB}. 
\DIFaddbegin \DIFadd{The stochastic problem is implemented in \texttt{Python} version 3.12.7 modeled in \texttt{Pyomo} version 6.9.4 \citep{bynum2021pyomo,hart2011pyomo}, and solved with \texttt{Gurobi} 12.0.3 \citep{gurobi}.
}\DIFaddend Computations are performed on an Apple M4 Pro (12-core CPU). Illustrative figures are created using \verb|ggplot2| by \citet{ggplot2}. The code repository can be accessed on \url{https://github.com/dobbeltgaard/MarkovJump.git}.

\section*{Data statement}
Due to sensitivity of data, Bane NOR \DIFdelbegin \DIFdel{has been assured }\DIFdelend \DIFaddbegin \DIFadd{required }\DIFaddend that the names of the individual rail lines are anonymized. \DIFdelbegin \DIFdel{Raw data is confidential and can therefore not }\DIFdelend \DIFaddbegin \DIFadd{The raw data are confidential and therefore cannot }\DIFaddend be shared. \DIFdelbegin \DIFdel{Further information }\DIFdelend \DIFaddbegin \DIFadd{To support reproducibility, we provide a synthetic dataset in the same censored tuple format as the original data, with similar empirical distributions of initial states and observation intervals (Appendix \ref{sec:synthetic_data}). Further details }\DIFaddend on the defect data is available from the authors \DIFaddbegin \DIFadd{upon request}\DIFaddend .
...
\DIFaddbegin \bibitem[Bisgaard et~al., 2026]{Bisgaard2026}
\DIFadd{Bisgaard, A.~S., M}{\DIFadd{\o}}\DIFadd{ller, J.~K., Madsen, H., and Ritschel, T. K.~S. (2026).
}\newblock \DIFadd{Hidden }{\DIFadd{Markov}} \DIFadd{models for bounded, inflated time series: Forecasting
  icing on wind turbine blades.
}\newblock {\em \DIFadd{Wind Energy}}\DIFadd{.
}\newblock \DIFadd{Article ID: WE70110.
}

\DIFaddend \bibitem[Bladt and Nielsen, 2017]{bladt_nielsen}
...
\DIFaddbegin \bibitem[Bynum et~al., 2021]{bynum2021pyomo}
\DIFadd{Bynum, M.~L., Hackebeil, G.~A., Hart, W.~E., Laird, C.~D., Nicholson, B.~L.,
  Siirola, J.~D., Watson, J.-P., and Woodruff, D.~L. (2021).
}\newblock {\em \DIFadd{Pyomo--optimization modeling in }{\DIFadd{Python}}}\DIFadd{, volume~67.
}\newblock \DIFadd{Springer Science \& Business Media, third edition.
}

\DIFaddend \bibitem[Cannon et~al., 2003]{cannon2003a}
...
\DIFaddbegin \bibitem[Frydman and Surya, 2024]{frydman2024}
\DIFadd{Frydman, H. and Surya, B.~A. (2024).
}\newblock \DIFadd{Estimation in a general mixture of }{\DIFadd{Markov}} \DIFadd{jump processes.
}\newblock {\em \DIFadd{Canadian Journal of Statistics}}\DIFadd{, 52(4):e11814.
}

\DIFaddend \bibitem[Fushiki, 2011]{fushiki2011a}
Fushiki, T. (2011).
\newblock Estimation of prediction error by using k-fold cross-validation.
\newblock {\em Statistics and Computing}, 21(2):137--146.

\DIFaddbegin \bibitem[Gay, 1990]{Gay1990}
\DIFadd{Gay, D.~M. (1990).
}\newblock \DIFadd{Usage summary for selected optimization routines.
}\newblock \DIFadd{Computing Science Technical Report 153, AT\&T Bell Laboratories,
  Murray Hill, NJ.
}\newblock \DIFadd{Accessed 2026-02-26.
}

\DIFaddend \bibitem[Gebetsberger et~al., 2018]{gebetsberger2018a}
...
\DIFaddbegin \bibitem[{Gurobi Optimization, LLC}, 2026]{gurobi}
{\DIFadd{Gurobi Optimization, LLC}} \DIFadd{(2026).
}\newblock {\DIFadd{Gurobi Optimizer Reference Manual}}\DIFadd{.
}

\DIFaddend \bibitem[Hamill, 1997]{hamill1997}
Hamill, T.~M. (1997).
\newblock Reliability diagrams for multicategory probabilistic forecasts.
\newblock {\em Weather and Forecasting}, 12(4):736 -- 741.

\DIFaddbegin \bibitem[Hart et~al., 2011]{hart2011pyomo}
\DIFadd{Hart, W.~E., Watson, J.-P., and Woodruff, D.~L. (2011).
}\newblock \DIFadd{Pyomo: }{\DIFadd{Modeling}} \DIFadd{and solving mathematical programs in }{\DIFadd{Python}}\DIFadd{.
}\newblock {\em \DIFadd{Mathematical Programming Computation}}\DIFadd{, 3(3):219--260.
}

\DIFaddend \bibitem[Hersbach, 2000]{Hersbach2000}
Hersbach, H. (2000).
...
\newblock Comparing homogeneous and inhomogeneous time \DIFdelbegin \DIFdel{markov }\DIFdelend \DIFaddbegin {\DIFadd{Markov}} \DIFaddend chains for
...
\newblock Infrastructure deterioration modeling with an inhomogeneous
  continuous time \DIFdelbegin \DIFdel{markov }\DIFdelend \DIFaddbegin {\DIFadd{Markov}} \DIFaddend chain: A latent state approach with analytic
  transition probabilities.
\newblock {\em Computer-Aided Civil and Infrastructure Engineering},
  38(13):1730--1748.

\DIFaddbegin \bibitem[Park and Jun, 2009]{park2009a}
\DIFadd{Park, H.-S. and Jun, C.-H. (2009).
}\newblock \DIFadd{A simple and fast algorithm for k-medoids clustering.
}\newblock {\em \DIFadd{Expert Systems With Applications}}\DIFadd{, 36(2):3336--3341.
}

\DIFaddend \bibitem[Podofillini et~al., 2006]{podofillini2006a}
...
\DIFaddbegin \bibitem[Powell, 2018]{powell2018a}
\DIFadd{Powell, W.~B. (2018).
}\newblock \DIFadd{A unified framework for stochastic optimization.
}\newblock {\em \DIFadd{European Journal of Operational Research}}\DIFadd{, 275(3):795--821.
}

\DIFaddend \bibitem[{R Core Team}, 2023]{R_stats}
{R Core Team} (2023).
...
\DIFaddbegin \bibitem[Sheather, 2009]{sheather2009a}
\DIFadd{Sheather, S. (2009).
}\newblock {\em \DIFadd{A Modern Approach to Regression with }{\DIFadd{R}}}\DIFadd{.
}\newblock \DIFadd{Springer Texts in Statistics. Springer, New York, NY.
}

\DIFaddend \bibitem[Sherlock, 2021]{sherlock2021a}
Sherlock, C. (2021).
\newblock Direct statistical inference for finite \DIFdelbegin \DIFdel{markov }\DIFdelend \DIFaddbegin {\DIFadd{Markov}} \DIFaddend jump processes via
  the matrix exponential.
\newblock {\em Computational Statistics}, 36(4):2863--2887.

\DIFaddbegin \bibitem[Shireman et~al., 2016]{shireman2016a}
\DIFadd{Shireman, E.~M., Steinley, D., and Brusco, M.~J. (2016).
}\newblock \DIFadd{Local optima in mixture modeling.
}\newblock {\em \DIFadd{Multivariate Behavioral Research}}\DIFadd{, 51(4):466--481.
}

\DIFaddend \bibitem[Sun and Vatn, 2024]{sun2024a}
...
\DIFaddbegin \bibitem[Thygesen, 2023]{thygesen2023a}
\DIFadd{Thygesen, U.~H. (2023).
}\newblock {\em \DIFadd{Stochastic Differential Equations for Science and Engineering}}\DIFadd{.
}\newblock \DIFadd{Taylor \& Francis.
}

\DIFaddend \bibitem[Venables and Ripley, 2002]{mass_R}
...
\newblock Simulation of railway drainage asset service condition degradation in
  the \DIFdelbegin \DIFdel{uk using a markov }\DIFdelend \DIFaddbegin {\DIFadd{UK}} \DIFadd{using a }{\DIFadd{Markov}} \DIFaddend chain–based approach.
...
\newblock \DIFdelbegin \DIFdel{Markov }\DIFdelend \DIFaddbegin {\DIFadd{Markov}} \DIFaddend chain modeling for predicting the service life of buildings
...
\section{\DIFadd{Estimation diagnostics of the Markov jump processes}}
\label{sec:diagnostics_estimation}

\begin{table}[h!]
\centering
\caption{\DIFaddFL{Akaike information criterion (AIC; computed from the log-likelihood at the score-optimal parameters) and per-observation gradient norm of the score objective.}}
\label{tab:AIC_tab}
\scriptsize
\input{AIC_tab}
\end{table}
...
%DIF > \section{Estimation diagnostics of the Markov jump processes} \label{sec:diagnostics_estimation}
%DIF > 
%DIF > \begin{table}[h!]  
%DIF > \centering
%DIF > \caption{Akaike information criterion (AIC; computed from the log-likelihood at the score-optimal parameters) and per-observation gradient norm of the score objective, reported by fold in the cross-validation.} \label{tab:AIC_tab}
%DIF > \centering
%DIF > \scriptsize
%DIF > \input{AIC_tab}
%DIF > %    \resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{\input{AIC_tab}}
%DIF > \end{table} 

\newpage

\section{\DIFadd{Transition residuals and sharpness through entropy}} \label{sec:transition_L1}  \label{sec:sharpness}
\DIFadd{Transition residual matrices  $\bm R$ are computed using held-out predictions from the cross-validation procedure in \cref{sec:prediction_error}.
We report the L1-norm of the residual matrix obtained by pooling observed transition counts and pooled model-implied expected counts across folds, i.e. $|| \bm R || = \sum_{i,j} |  R_{i,j}|$ for $i,j \in \mathcal J$. %DIF > Lower values indicate closer agreement between observed and model-implied transition frequencies.
...
\begin{table}[h!]  
\centering
\caption{\DIFaddFL{L1-norm of transition residuals $
...
\section{\DIFadd{Synthetic data}} \label{sec:synthetic_data}
\DIFadd{As the raw defect data provided by the infrastructure manager are confidential, they cannot be shared. To support reproducibility of the proposed methods of this study, we provide a synthetic dataset in the same format (with censoring) as the original transition data, i.e., tuples $\{(y_1^{(s)},y_2^{(s)},\tau^{(s)},\bm z^{(s)})\}_{s \in \mathcal S}$, which can be used to run all proposed estimation and prediction procedures.
}

\DIFadd{The synthetic data is generated as follows.
}\begin{itemize}
\item \DIFadd{An MJP with a chosen model specification is estimated on the confidential defect data using the methods, so that model parameters $\widehat{\bm \theta}$ are obtained.
}\item \DIFadd{$n$ initial states $y_1'$ are sampled independently from the empirical distribution of the observed initial states in the confidential dataset, i.e.
$
y_1' \sim \text{Categorical}(\widehat{\bm p})$ where $\widehat p_i = \frac{1}{| \mathcal S|} \sum_{s\in\mathcal S} \mathds{1}_{y_1^{(s)}=\;i}
$ for $i \in \mathcal J$. 
%DIF > $\left(\widehat p_i = \frac{1}{|\mathcal S|}\sum_{s\in\mathcal S}\mathds{1}_{y_1^{(s)}=i}\right)$
}\item \DIFadd{$n$ observation intervals $\tau'_1,\dots,\tau'_n$ are sampled independently by bootstrap resampling from the observed intervals, i.e.,
$
...
}\item \DIFadd{The transient distribution for each sampled defect under the model and the estimated parameters is then used as the categorical distribution for which an ending state is sampled, i.e., 
$
...
\DIFadd{We generate a synthetic dataset using the $\bm{A}_{\RN{3}}$ parameterization (\cref{sec:generator_params}), warping (\cref{sec:warping}), and global regression (\cref{eq:trans_rates_covariates} in \cref{sec:regression}) with $n = 10000$.
}

\DIFadd{This synthetic dataset serves two purposes: 1) the proposed methods can be tested on data with the same structure as the confidential dataset; 2) it enables numerical verification that the estimation procedure can recover the data-generating parameters and that estimation error decreases as $n$ increases.
}
...
\section{\DIFadd{Details on the stochastic optimization program}} \label{sec:stochastic_optimization_details}
\DIFadd{Here, we provide additional details on the stochastic optimization program described in \cref{sec:proposition_for_use}.
}\paragraph{\DIFadd{Sampling and tree reduction.}}
\DIFadd{We consider a three-stage horizon with decision times
...
\paragraph{\DIFadd{Encoding of endogenous information.}}
\DIFadd{The reduced tree represents future joint defect scenarios, while inspection changes what is known later.
...
\paragraph{\DIFadd{Numerical details.}}
\DIFadd{The reduced-tree architecture is specified by $(K_2,K_3)$, where $K_2$ is the number of stage-2 clusters and $K_3$ the number of stage-3 children per stage-2 cluster.
...
\DIFadd{\Cref{tab:setup} displays the parameters used to run the optimization program.
}\begin{table}[htb!]
\centering
\caption{\DIFaddFL{Experimental setup.}} \label{tab:setup}
\begin{tabular}{p{0.43\linewidth}p{0.47\linewidth}}
\toprule
\DIFaddFL{Item }& \DIFaddFL{Value }\\
\midrule
\DIFaddFL{Number of defects }& \DIFaddFL{\(10\) }\\
\DIFaddFL{Decision times }& \DIFaddFL{\((0.5,\,1.0,\,1.5)\) }\\
\DIFaddFL{Training / validation / test paths }& \DIFaddFL{\(10^6 / 10^5 / 10^5\) }\\
\DIFaddFL{Training / validation / test seeds }& \DIFaddFL{\(42 / 43 / 44\) }\\
\DIFaddFL{Inspection cost \(c_{\mathrm{ins}}\) }& \DIFaddFL{\(150\) }\\
\DIFaddFL{Failure cost \(c_{\mathrm{fail}}\) }& \DIFaddFL{\(200{,}000\) }\\
\DIFaddFL{Maintenance costs for classes in $\mathcal E$ }& \DIFaddFL{\(2000,\,3500,\,4500,\,8000,\,10000\) }\\
\DIFaddFL{Stage budgets }& \DIFaddFL{\((20000,\,19500,\,18000)\) }\\
\DIFaddFL{Critical classes $\mathcal E_{\text{critical}}$ }& \DIFaddFL{\(\{ \text{Class } $1$, \text{Class } $0$\}\) }\\
\bottomrule
\end{tabular}
\end{table}

\DIFadd{\Cref{tab:reduced_tree} displays the parameters used to reduce the full tree and obtained results from solving the program.
}\begin{table}[htb!]
\centering
\caption{\DIFaddFL{Summary for the selected reduced-tree architecture.}} \label{tab:reduced_tree}
\begin{tabular}{p{0.55\linewidth}p{0.30\linewidth}}
\toprule
\DIFaddFL{Item }& \DIFaddFL{Value }\\
\midrule
\DIFaddFL{Selected architecture $(K_2 \times K_3)$ }& \DIFaddFL{$20\times 8$ }\\
\DIFaddFL{Realized stage-2 / stage-3 nodes }& \DIFaddFL{$20 / 160$ }\\
\DIFaddFL{Validation mean total cost (s.e.) }& \DIFaddFL{$105063.69\ (364.86)$ }\\
\DIFaddFL{Test mean total cost }& \DIFaddFL{$104854.52$ }\\
\DIFaddFL{Stage-3 critical-count total variation }& \DIFaddFL{$6.9\times 10^{-5}$ }\\
\DIFaddFL{Stage-1 budget usage (\%) }& \DIFaddFL{$96.64\%$ }\\
\bottomrule
\end{tabular}
\end{table}
\DIFaddend 
