...
brier.estim.cov <- function(m, s1, s2, u, z, beta, pvalues.bin = F, method = NULL) {
  ...
  mod <- optim(
    par = beta, fn = discrete_brier_cpp_cov, m = m,
    s1 = s1, s2 = s2, u = u, z = z,
...
loglik.brier.rps.estim.cov.A2 <- function(m, s1, s2, u, z, beta, pvalues.bin = F, method = NULL) {
  start_time <- Sys.time() # Start of run time
  if (is.null(method) | method == "Nelder-Mead") { # Nelder-Mead = standard method
    method <- "Nelder-Mead"
    control <- list(maxit = 20000)
  ...
  mod <- optim(
    par = beta, fn = discrete_MLE_brier_RPS_cpp_cov_A2, m = m,
    s1 = s1, s2 = s2, u = u, z = z,
    control = control, hessian = pvalues.bin,
    method = method) # optimize
  end_time <- Sys.time() # End of run time
...
brier.loglik.estim.cov <- function(m, s1, s2, u, z, beta, pvalues.bin = F, method = NULL) {
  ...
  mod <- optim(
    par = beta, fn = discrete_brier_loglik_cpp_cov, m = m,
    s1 = s1, s2 = s2, u = u, z = z,
    control = control, hessian = pvalues.bin,
    method = method) # optimize
  end_time <- Sys.time() # End of run time
...
brier.estim <- function(m, s1, s2, u, beta, pvalues.bin = F, method = NULL) {
  ...
  mod <- optim(
    par = beta, fn = discrete_brier_cpp, m = m,
    s1 = s1, s2 = s2, u = u,
    control = control, hessian = pvalues.bin,
    method = method) # optimize
  end_time <- Sys.time() # End of run time
