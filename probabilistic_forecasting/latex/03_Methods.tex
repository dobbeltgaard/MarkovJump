...
To allow such flexibility, we propose temporal warping through state-specific weights $\xi_i$.
...
If all $\xi_i = 1$, the model reduces to one without warping, i.e., $\widetilde\tau^{(s)} = \tau^{(s)}$. The weight for the absorbing state is set to $1$ to ensure that arrival at the absorbing state concludes the degradation process in the warped time domain as well.
We compare models with and without temporal warping in terms of predictive accuracy. For notational simplicity, we use $\tau'$ to denote elapsed time in both the standard covariate-dependent MJP and its temporally warped version.
...
The total number of base rate parameters is $2m - 3$. With covariates and warping, it becomes $3m - 4 + p$ parameters in total.
...
Our implementation is flexible: users may choose the scoring rule (log-score and/or ranked probability score), select among the five generator parameterizations in \cref{sec:generator_params}, and optionally incorporate covariates and temporal warping (\cref{sec:warping}).
