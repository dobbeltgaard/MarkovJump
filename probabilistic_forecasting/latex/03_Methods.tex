...
\end{align*}
where $\widehat{\bm{\theta}}_n  \rightarrow \bm{\theta}_0$ as $n \rightarrow \infty$ with $\bm{\theta}_0$ denoting the true set of model parameters.
...
\end{align}
where $D_i(P_{\bm{\theta}}, Y^{(s)})$ for $i = 1,2$ are the two different metrics considered in this study. If deemed relevant, weights can be assigned to each of the terms. The following describes these metrics.
...
Unlike the log-score, the RPS reflects that misclassifying a crack by one class is less serious than misclassifying it by several classes. We therefore suggest placing greater emphasis on RPS in studies of degradation. 
