...
K = 4; mixlen=K-1; cov_len=length(exo.cols); ridge = 1e-4;
...
for(i in 1:5){
  ...
  write.csv(obs, file = paste0("predictions_V2/", "obs", "_fold_", i, ".csv"), row.names = FALSE)
  ...
  for(gen in parameterizations){
    num = -1; 
    if(gen == "gerlang"){generator_type = 0; beta_base = rep(num,m-1); }
    if(gen == "gerlang_relax"){generator_type = 1; beta_base = rep(num,m-1); }
    if(gen == "free_upper_tri"){generator_type = 2; beta_base = rep(num,m*(m-1)/2); }
    if(gen == "bidiagonal"){generator_type = 3; beta_base = rep(num,2*m-3); }
    if(gen == "tridiagonal"){generator_type = 4; beta_base = rep(num,3*m-6); }
        ...
        for(covslink in links){
          for(warping in warpings){
            ...
            for(score in scores){
              ...
              nam = paste(gen,paste0("cov",cov),baselink,covslink,score,warping,paste0("mixture",K),sep = "_") #!
              print(nam)
              
              if(cov == 2 ){ beta = c(rep(0,mixlen), rep(beta_base,K), xi, rep(0,length(exo.cols) * (m-1))); state_covs = T; } #!
              if(cov == 1 ){ beta = c(rep(0,mixlen), rep(beta_base,K), xi, rep(0,length(exo.cols) )); state_covs = F; } #!
              if(cov == 0 ){ beta = c(rep(0,mixlen), rep(beta_base,K), xi); state_covs = F; } #!
              
              data.train <- list(s1 = d.train$s1,s2 = d.train$s2,u = d.train$t,z = as.matrix(d.train[, exo.cols]),m = m,generator_type = generator_type,cov_type = as.integer(cov),use_log_score = as.integer(log_bin),use_rps_score = as.integer(rps_bin), use_brier_score = 0, K = as.integer(K), ridge = ridge)
              ...
              print(paste("Convergence:", foo$message))
              rm(l); invisible(gc())
              
              #compute AIC
              data.train.AIC <- list(s1 = d.train$s1,s2 = d.train$s2,u = d.train$t,z = as.matrix(d.train[, exo.cols]),m = m,generator_type = generator_type,cov_type = as.integer(cov),use_log_score = as.integer(1),use_rps_score = as.integer(0), use_brier_score = 0, K = as.integer(K), ridge = 0)
              ...
              pred = MJP_predict(m = m, s1 = d.test$s1, u = d.test$t, pars = foo$par, z = as.matrix(d.test[,exo.cols]), generator = gen, link_type_base = baselink, link_type_covs = covslink, covs_bin = isTRUE(cov>0), transient_dist_method = "pade", warping = warp, state_covs = state_covs, mixture = TRUE, K = K) #!
